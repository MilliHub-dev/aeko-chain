...
use {
    ...
    aeko_storage_bigtable::CredentialType,
...
async fn upload(
    ...
    force_reupload: bool,
    config: aeko_storage_bigtable::LedgerStorageConfig,
) -> Result<(), Box<dyn std::error::Error>> {
    let bigtable = aeko_storage_bigtable::LedgerStorage::new_with_config(config)
        .await
    ...
    while starting_slot <= ending_slot {
        ...
        let last_slot_checked = aeko_ledger::bigtable_upload::upload_confirmed_blocks(
            blockstore.clone(),
            bigtable.clone(),
            starting_slot,
            current_ending_slot,
...
async fn delete_slots(
    ...
    aeko_ledger::bigtable_delete::delete_confirmed_blocks(bigtable, slots, dry_run).await
}

async fn first_available_block(
    config: aeko_storage_bigtable::LedgerStorageConfig,
) -> Result<(), Box<dyn std::error::Error>> {
    let bigtable = aeko_storage_bigtable::LedgerStorage::new_with_config(config).await?;
...
async fn compare_blocks(
    ...
    limit: usize,
    config: aeko_storage_bigtable::LedgerStorageConfig,
    ref_config: aeko_storage_bigtable::LedgerStorageConfig,
) -> Result<(), Box<dyn std::error::Error>> {
    let reference_bigtable = aeko_storage_bigtable::LedgerStorage::new_with_config(ref_config)
        .await
    ...
    let owned_bigtable = aeko_storage_bigtable::LedgerStorage::new_with_config(config)
        .await
                    ...
                    if args.dry_run {
                        ...
                        let confirmed_block =
                            match source_bigtable_clone.get_confirmed_block(slot).await {
                                ...
                                Err(aeko_storage_bigtable::Error::BlockNotFound(slot)) => {
                                    debug!("block not found, slot: {}", slot);
                                    block_not_found_slots_clone.lock().unwrap().push(slot);
                                    continue;
                                }
...
) -> aeko_storage_bigtable::Result<aeko_storage_bigtable::LedgerStorage> {
    if let Some(endpoint) = args.emulated_source {
        aeko_storage_bigtable::LedgerStorage::new_for_emulator(
            &args.instance_name,
            &args.app_profile_id,
            &endpoint,
            args.timeout,
        ...
        aeko_storage_bigtable::LedgerStorage::new_with_config(
            aeko_storage_bigtable::LedgerStorageConfig {
            ...
            SubCommand::with_name("bigtable")
                ...
                .arg(
                    Arg::with_name("rpc_bigtable_instance_name")
                        ...
                        .default_value(aeko_storage_bigtable::DEFAULT_INSTANCE_NAME)
                ...
                .arg(
                    Arg::with_name("rpc_bigtable_app_profile_id")
                        ...
                        .default_value(aeko_storage_bigtable::DEFAULT_APP_PROFILE_ID)
...
pub fn bigtable_process_command(ledger_path: &Path, matches: &ArgMatches<'_>) {
    ...
    let instance_name = get_global_subcommand_arg(
        ...
        aeko_storage_bigtable::DEFAULT_INSTANCE_NAME,
    );
    let app_profile_id = get_global_subcommand_arg(
        matches,
        sub_matches,
        "rpc_bigtable_app_profile_id",
        aeko_storage_bigtable::DEFAULT_APP_PROFILE_ID,
    );
    ...
    let future = match (subcommand, sub_matches) {
        ("upload", Some(arg_matches)) => {
            ...
            let config = aeko_storage_bigtable::LedgerStorageConfig {
                read_only: false,
                instance_name,
                app_profile_id,
                ..aeko_storage_bigtable::LedgerStorageConfig::default()
            };
        ...
        ("compare-blocks", Some(arg_matches)) => {
            ...
            let ref_config = aeko_storage_bigtable::LedgerStorageConfig {
                read_only: true,
