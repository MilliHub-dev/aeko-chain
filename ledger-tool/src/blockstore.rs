...
use {
    ...
    regex::Regex,
    serde_json::json,
    aeko_clap_utils::{hidden_unless_forced, input_validators::is_slot},
...
fn analyze_column<
    C: aeko_ledger::blockstore_db::Column + aeko_ledger::blockstore_db::ColumnName,
>(
    db: &Database,
    name: &str,
) -> Result<()> {
...
fn analyze_storage(database: &Database) -> Result<()> {
    use aeko_ledger::blockstore_db::columns::*;
...
fn raw_key_to_slot(key: &[u8], column_name: &str) -> Option<Slot> {
    use aeko_ledger::blockstore_db::columns as cf;
    match column_name {
...
fn slot_contains_nonvote_tx(blockstore: &Blockstore, slot: Slot) -> bool {
    ...
    let contains_nonvote = entries
        ...
        .any(|program_id| *program_id != aeko_vote_program::id());
    contains_nonvote
}
...
pub fn blockstore_subcommands<'a, 'b>(hidden: bool) -> Vec<App<'a, 'b>> {
    ...
    vec![
        ...
        SubCommand::with_name("latest-optimistic-slots")
            ...
            // such that the subcommand will be visible as the top level of aeko-ledger-tool
            .arg(
...
pub mod tests {
    use {
        super::*,
        aeko_ledger::{blockstore::make_many_slot_entries, get_tmp_ledger_path_auto_delete},
    };
