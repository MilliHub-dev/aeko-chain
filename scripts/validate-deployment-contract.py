...
def main() -> int:
    ...
    require(coolify.count("source: /data/aeko/keys") >= 5, "Coolify services must share the fixed host key bind source")
    require(coolify.count("read_only: true") >= 4, "Coolify runtime key mounts must remain read-only")
    require("AEKO_KEYS_SOURCE: /data/aeko/keys" in coolify_preflight, "Coolify preflight must report the fixed host key source")
