...
if [[ -z $1 ]]; then
  packages=(--lib --all --exclude aeko-local-cluster)
else
  packages=("$@")
fi
...
export aeko_TEST_ACCOUNTS_INDEX_MEMORY_LIMIT_MB=10000 
...
RUST_LOG=aeko=trace _ "$cargo" nightly test --jobs "$JOBS" --target-dir target/cov --no-run "${packages[@]}"
if RUST_LOG=aeko=trace _ "$cargo" nightly test --jobs "$JOBS" --target-dir target/cov "${packages[@]}" 2> target/cov/coverage-stderr.log; then
  test_status=0
else
  test_status=$?
