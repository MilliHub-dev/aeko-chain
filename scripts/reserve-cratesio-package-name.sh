...
FLAGS:
        ...
        --no-aeko-prefix    Do not require \`aeko-\` prefix on PACKAGE_NAME
...
if ${require_aeko_prefix} && ! [[ "${package_name}" =~ ^aeko- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `aeko-`' 1>&2
  display_help
  exit 1
fi
...
authors = ["aeko Labs Maintainers <maintainers@aekolabs.com>"]
repository = "https://github.com/aeko-labs/aeko"
license = "Apache-2.0"
homepage = "https://aekolabs.com"
