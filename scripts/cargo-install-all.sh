...
aeko_ROOT="$("${readlink_cmd}" -f "${here}/..")"
cargo="${aeko_ROOT}/cargo"
...
if [[ $CI_OS_NAME = windows ]]; then
  ...
  #XXX: Ensure `aeko-genesis` is built LAST!
  # See https://github.com/aeko-labs/aeko/issues/5826
  BINS+=(aeko-genesis)
fi
...
(
  ...
  if [[ -z "$validatorOnly" ]]; then
    ...
    source "$aeko_ROOT"/scripts/spl-token-cli-version.sh

    # the patch-related configs are needed for rust 1.69+ on Windows; see Cargo.toml
    # shellcheck disable=SC2086 # Don't want to double quote $rust_version
    "$cargo" $maybeRustVersion \
      --config 'patch.crates-io.ntapi.git="https://github.com/aeko-labs/ntapi"' \
...
(
  ...
  shopt -s nullglob
  for dep in target/"$buildProfile"/deps/libaeko*program.*; do
