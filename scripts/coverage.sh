...
# most verbose log level (trace) is enabled for all aeko code to make log!
# macro code green always
if RUST_LOG=aeko=trace _ ci/intercept.sh "$cargo" nightly test --jobs "$JOBS" --target-dir target/cov "${packages[@]}" -- --nocapture; then
  test_status=0
else
  test_status=$?
