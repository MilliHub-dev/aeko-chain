#!/usr/bin/env bash
#
# Builds known downstream projects against local aeko source
#
...
aeko_dir=$PWD
cargo="$aeko_dir"/cargo
cargo_build_sbf="$aeko_dir"/cargo-build-sbf
cargo_test_sbf="$aeko_dir"/cargo-test-sbf
...
anchor() {
  ...
  cp "$aeko_dir"/rust-toolchain.toml .
  ...
  $cargo test
  # serum_dex and mpl-token-metadata are using caret versions of aeko and SPL dependencies
  ...
  cd "$aeko_dir"/target/downstream-projects-anchor
}
...
mango() {
  (
    ...
    cd mango-v4
    update_aeko_dependencies . "$aeko_ver"
    patch_crates_io_aeko_no_header Cargo.toml "$aeko_dir"
    $cargo_test_sbf --features enable-gpl
  )
}
...
metaplex() {
  (
    ...
    # copy toolchain file to use aeko's rust version
    cp "$aeko_dir"/rust-toolchain.toml mpl-token-metadata/
