...
#     git clone https://github.com/aeko-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
    git clone git@github.com:aeko-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
  ...
  get_net_launch_software_version_launch_args "$CHANNEL" "aeko-release" version_args
...
[[ -n $INFLUX_HOST ]] || INFLUX_HOST=https://internal-metrics.aeko.com:8086
...
if [[ -z $AEKO_METRICS_CONFIG ]]; then
  if [[ -z $AEKO_METRICS_PARTIAL_CONFIG ]]; then
    echo AEKO_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export AEKO_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$AEKO_METRICS_PARTIAL_CONFIG"
fi
echo "AEKO_METRICS_CONFIG: $AEKO_METRICS_CONFIG"
