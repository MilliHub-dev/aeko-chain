...
fn main() -> Result<()> {
    ...
    let authority = match env::var("AEKO_AUTHORITY_KEYPAIR") {
        Ok(path) if !path.trim().is_empty() => read_keypair_file(path.trim())
            .map_err(|error| anyhow!("failed to read authority keypair at {path}: {error}"))?,
    ...
    let live_genesis = client
        .get_genesis_hash()
        .context("reading validator genesis hash for SocialFi lifecycle")?
        .to_string();
    let reset_requested = parse_bool_flag("AEKO_RESET_LEDGER")?;
    ...
    let state_rent = with_retries("state rent", || {
        client
            .get_minimum_balance_for_rent_exemption(STATE_ACCOUNT_SPACE as usize)
            .map_err(anyhow::Error::from)
    })?;
    let vault_rent = with_retries("vault rent", || {
        client
    ...
    ensure_program_vault(
        &client,
        &payer,
        &rewards_treasury,
        &aeko_social_rewards_program::id(),
        vault_rent,
        seed_lamports("AEKO_REWARDS_TREASURY_SEED_LAMPORTS")?,
    ...
    ensure_program_vault(
        ...
        &aeko_social_rewards_program::id(),
        vault_rent,
        seed_lamports("AEKO_REWARD_VAULT_SEED_LAMPORTS")?,
    ...
    ensure_program_vault(
        ...
        &aeko_social_staking_program::id(),
        vault_rent,
        seed_lamports("AEKO_STAKE_REWARD_VAULT_SEED_LAMPORTS")?,
    ...
    let posts_data = aeko_social_posts_program::state::SocialPostsStateAccount::new(
    ...
    create_and_init(
        ...
        state_rent,
        aeko_social_posts_program::instruction::initialize_state(
            &aeko_social_posts_program::id(),
            &posts_state.pubkey(),
            &payer.pubkey(),
            &authority.pubkey(),
    ...
    create_and_init(
        ...
        &aeko_social_rewards_program::id(),
        state_rent,
        aeko_social_rewards_program::instruction::initialize_config(
            &aeko_social_rewards_program::id(),
    ...
    create_and_init(
        ...
        &aeko_social_staking_program::id(),
        state_rent,
        aeko_social_staking_program::instruction::initialize_config(
            &aeko_social_staking_program::id(),
    ...
    create_and_init(
        ...
        state_rent,
        aeko_social_anti_spam_program::instruction::initialize_config(
            &aeko_social_anti_spam_program::id(),
    ...
    create_and_init(
        ...
        &aeko_social_monetization_program::id(),
        state_rent,
        aeko_social_monetization_program::instruction::initialize_config(
            &aeko_social_monetization_program::id(),
    ...
    submit_authority_instruction(
        &client,
        &payer,
        &authority,
        aeko_social_rewards_program::instruction::update_vaults(
            &aeko_social_rewards_program::id(),
            &rewards_state.pubkey(),
            &authority.pubkey(),
    ...
    submit_authority_instruction(
        ...
        &payer,
        &authority,
        aeko_social_staking_program::instruction::update_vaults(
            &aeko_social_staking_program::id(),
            &staking_state.pubkey(),
            &authority.pubkey(),
            &stake_vault.pubkey(),
    ...
    submit_authority_instruction(
        ...
        &payer,
        &authority,
        aeko_social_monetization_program::instruction::update_treasury(
            &aeko_social_monetization_program::id(),
            &monetization_state.pubkey(),
            &authority.pubkey(),
...
fn ensure_program_vault(
    ...
    .value
    {
        Some(account) => {
            if account.owner != *owner {
                return Err(anyhow!(
                    "[{label}] vault {pubkey} owner {} does not match {owner}",
                    account.owner
                ));
            }
            if !account.data.is_empty() {
                return Err(anyhow!("[{label}] vault {pubkey} must have zero data"));
            }
            if account.lamports < rent.saturating_add(desired_seed) {
                let top_up = rent
                    .saturating_add(desired_seed)
                    .saturating_sub(account.lamports);
                submit_instruction(
                    client,
                    payer,
                    &[payer],
                    system_instruction::transfer(&payer.pubkey(), &pubkey, top_up),
                    &format!("top up {label}"),
                )?;
            }
            return Ok(());
    ...
    submit_instruction(
        ...
        payer,
        &[payer, vault],
        system_instruction::create_account(&payer.pubkey(), &pubkey, lamports, 0, owner),
        &format!("create {label}"),
    )
}

fn submit_authority_instruction(
    client: &RpcClient,
    payer: &Keypair,
    authority: &Keypair,
    instruction: Instruction,
...
fn create_and_init(
    ...
    for attempt in 1..=SEND_MAX_ATTEMPTS {
        ...
        match client.send_and_confirm_transaction(&tx) {
            ...
            Err(error) => {
                if existing_state_is_initialized(
                    client,
                    &state_pubkey,
                    program_id,
                    label,
...
fn existing_state_is_initialized(
    ...
    match response.value {
        ...
        Some(_) => Err(anyhow!(
            "[{label}] state {state_pubkey} exists but is not initialized"
        )),
    }
}
...
fn rewards_state_initialized(data: &[u8]) -> Result<bool> {
    Ok(
        aeko_social_rewards_program::state::SocialRewardsStateAccount::deserialize_padded(data)
            .map_err(|_| anyhow!("invalid social-rewards state"))?
            .is_initialized,
    )
}
fn staking_state_initialized(data: &[u8]) -> Result<bool> {
    Ok(
        aeko_social_staking_program::state::SocialStakingStateAccount::deserialize_padded(data)
            .map_err(|_| anyhow!("invalid social-staking state"))?
            .is_initialized,
    )
}
fn anti_spam_state_initialized(data: &[u8]) -> Result<bool> {
    Ok(
        aeko_social_anti_spam_program::state::SocialAntiSpamStateAccount::deserialize_padded(data)
            .map_err(|_| anyhow!("invalid social-anti-spam state"))?
            .is_initialized,
    )
}
fn monetization_state_initialized(data: &[u8]) -> Result<bool> {
    Ok(aeko_social_monetization_program::state::SocialMonetizationStateAccount::deserialize_padded(data).map_err(|_| anyhow!("invalid social-monetization state"))?.is_initialized)
}
...
fn seed_lamports(name: &str) -> Result<u64> {
    env::var(name)
        ...
        .parse::<u64>()
        .with_context(|| format!("{name} must be a non-negative lamport amount"))
}
...
fn parse_platform_fee_bps() -> Result<u16> {
    ...
    let fee = value
        .parse::<u16>()
        .with_context(|| format!("AEKO_PLATFORM_FEE_BPS={value:?} is not a u16"))?;
    if fee > 10_000 {
        return Err(anyhow!("AEKO_PLATFORM_FEE_BPS must be <= 10000"));
    }
    Ok(fee)
}
