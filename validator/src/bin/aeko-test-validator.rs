use {
    ...
    aeko_core::consensus::tower_storage::FileTowerStorage,
    aeko_faucet::faucet::run_local_faucet_with_port,
    aeko_rpc::{
...
fn main() {
    ...
    let version = aeko_version::version!();
    ...
    aeko_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    info!("{} {}", crate_name!(), aeko_version::version!());
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        aeko_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        aeko_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
    ...
    let faucet_lamports = aeko_to_lamports(value_of(&matches, "faucet_aeko").unwrap());
    ...
    let faucet_per_time_cap = value_t!(matches, "faucet_per_time_aeko_cap", f64)
        .ok()
        .map(aeko_to_lamports);
    let faucet_per_request_cap = value_t!(matches, "faucet_per_request_aeko_cap", f64)
        .ok()
        .map(aeko_to_lamports);
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `aeko airdrop` localnet AEKO after creating one\n",
        );
    }
