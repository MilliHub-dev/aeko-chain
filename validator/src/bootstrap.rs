use {
    ...
    rayon::prelude::*,
    aeko_core::validator::{ValidatorConfig, ValidatorStartProgress},
    aeko_download_utils::{download_snapshot_archive, DownloadProgressRecord},
    aeko_genesis_utils::download_then_check_genesis_hash,
    aeko_gossip::{
...
fn verify_reachable_ports(
    ...
    aeko_net_utils::verify_reachable_ports(
...
fn check_vote_account(
    ...
    let vote_state = aeko_vote_program::vote_state::from(&vote_account);
...
pub fn attempt_download_genesis_and_snapshot(
    ...
    if let Some(url) = bootstrap_config.check_vote_account.as_ref() {
        ...
        .unwrap_or_else(|err| {
            ...
            // incorrect `aeko-validator` command-line arguments) rather than the
            // RPC node failing.
            //
...
fn download_snapshot(
    ...
    *start_progress.write().unwrap() = ValidatorStartProgress::DownloadingSnapshot {
    ...
    let desired_snapshot_hash = (
        ...
        aeko_runtime::snapshot_hash::SnapshotHash(desired_snapshot_hash.1),
    );
