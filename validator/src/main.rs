...
use {
    ...
    aeko_clap_utils::input_parsers::{keypair_of, keypairs_of, pubkey_of, value_of},
    aeko_core::{
    ...
    aeko_gossip::{cluster_info::Node, legacy_contact_info::LegacyContactInfo as ContactInfo},
    aeko_ledger::{
    ...
    aeko_perf::recycler::enable_recycler_warming,
    aeko_poh::poh_service,
    ...
    aeko_rpc_client_api::config::RpcLeaderScheduleConfig,
    aeko_runtime::{
    ...
    aeko_send_transaction_service::send_transaction_service,
...
fn get_cluster_shred_version(entrypoints: &[SocketAddr]) -> Option<u16> {
    ...
    for entrypoint in entrypoints {
        match aeko_net_utils::get_cluster_shred_version(entrypoint) {
...
pub fn main() {
    ...
    let cli_app = app(aeko_version, &default_args);
        ...
        ("set-public-address", Some(subcommand_matches)) => {
            let parse_arg_addr = |arg_name: &str, arg_long: &str| -> Option<SocketAddr> {
                subcommand_matches.value_of(arg_name).map(|host_port| {
                    aeko_net_utils::parse_host_port(host_port).unwrap_or_else(|err| {
                        eprintln!(
    ...
    info!("{} {}", crate_name!(), aeko_version);
    info!("Starting validator with: {:#?}", std::env::args_os());

    let cuda = matches.is_present("cuda");
    if cuda {
        aeko_perf::perf_libs::init_cuda();
        enable_recycler_warming();
    }

    aeko_core::validator::report_target_features();
    ...
    let bind_address = aeko_net_utils::parse_host(matches.value_of("bind_address").unwrap())
        .expect("invalid bind_address");
    let rpc_bind_address = if matches.is_present("rpc_bind_address") {
        aeko_net_utils::parse_host(matches.value_of("rpc_bind_address").unwrap())
            .expect("invalid rpc_bind_address")
    } else if private_rpc {
        aeko_net_utils::parse_host("127.0.0.1").unwrap()
    } else {
        bind_address
    };
    ...
    let entrypoint_addrs = values_t!(matches, "entrypoint", String)
        ...
        .into_iter()
        .map(|entrypoint| {
            aeko_net_utils::parse_host_port(&entrypoint).unwrap_or_else(|e| {
    ...
    let rpc_send_transaction_tpu_peers = matches
        ...
        .map(|values| {
            values
                .map(aeko_net_utils::parse_host_port)
    ...
    let dynamic_port_range =
        aeko_net_utils::parse_port_range(matches.value_of("dynamic_port_range").unwrap())
    ...
    let public_rpc_addr = matches.value_of("public_rpc_addr").map(|addr| {
        aeko_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if !matches.is_present("no_os_network_limits_test") {
        if SystemMonitorService::check_os_network_limits() {
            ...
            eprintln!("OS network limit test failed. See: https://docs.aeko.network/operations/guides/validator-start#system-tuning");
            exit(1);
        }
    }
    ...
    let gossip_host: IpAddr = matches
        ...
        .map(|gossip_host| {
            aeko_net_utils::parse_host(gossip_host).unwrap_or_else(|err| {
        ...
        .unwrap_or_else(|| {
            if !entrypoint_addrs.is_empty() {
                ...
                let gossip_host = order.into_iter().find_map(|i| {
                    ...
                    aeko_net_utils::get_public_ip_addr(entrypoint_addr).map_or_else(
                        |err| {
                            eprintln!(
    ...
    let gossip_addr = SocketAddr::new(
        ...
        value_t!(matches, "gossip_port", u16).unwrap_or_else(|_| {
            aeko_net_utils::find_available_port_in_range(bind_address, (0, 1)).unwrap_or_else(
                |err| {
    ...
    let public_tpu_addr = matches.value_of("public_tpu_addr").map(|public_tpu_addr| {
        aeko_net_utils::parse_host_port(public_tpu_addr).unwrap_or_else(|err| {
    ...
    let public_tpu_forwards_addr =
        matches
            ...
            .map(|public_tpu_forwards_addr| {
                aeko_net_utils::parse_host_port(public_tpu_forwards_addr).unwrap_or_else(|err| {
    ...
    aeko_metrics::set_host_id(identity_keypair.pubkey().to_string());
    aeko_metrics::set_panic_hook("validator", Some(String::from(aeko_version)));
    aeko_entry::entry::init_poh();
