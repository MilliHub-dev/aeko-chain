...
impl RestartLastVotedForkSlots {
    ...
    pub(crate) fn new_rand<R: Rng>(rng: &mut R, pubkey: Option<Pubkey>) -> Self {
        let pubkey = pubkey.unwrap_or_else(aeko_sdk::pubkey::new_rand);
...
impl RestartHeaviestFork {
    pub(crate) fn new_rand<R: Rng>(rng: &mut R, from: Option<Pubkey>) -> Self {
        let from = from.unwrap_or_else(aeko_sdk::pubkey::new_rand);
        Self {
            from,
...
mod test {
    use {
        ...
        aeko_sdk::{signature::Signer, signer::keypair::Keypair, timing::timestamp},
