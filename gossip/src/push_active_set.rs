        ...
        for (k, entry) in self.0.iter_mut().enumerate() {
            let weights: Vec<u64> = buckets
                ...
                .map(|&bucket| {
                    ...
                    // https://github.com/aeko-chain/solana/blob/81394cf92/gossip/src/received_cache.rs#L100-L105
...
fn get_stake_bucket(stake: Option<&u64>) -> usize {
    let stake = stake.copied().unwrap_or_default() / LAMPORTS_PER_AEKO;
...
mod tests {
    ...
    fn test_get_stake_bucket() {
        ...
        for (k, bucket) in buckets.into_iter().enumerate() {
            let stake = (k as u64) * LAMPORTS_PER_AEKO;
        ...
        for (stake, bucket) in [
            ...
            let stake = stake * LAMPORTS_PER_AEKO;
            assert_eq!(get_stake_bucket(Some(&stake)), bucket);
        }
        assert_eq!(
    ...
    fn test_push_active_set() {
        ...
        const MAX_STAKE: u64 = (1 << 20) * LAMPORTS_PER_AEKO;
