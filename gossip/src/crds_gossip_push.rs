...
mod tests {
    ...
    fn test_process_push_old_version() {
        let crds = RwLock::<Crds>::default();
        let push = CrdsGossipPush::default();
        let mut ci = ContactInfo::new_localhost(&aeko_sdk::pubkey::new_rand(), 0);
        ci.set_wallclock(1);
    ...
    fn test_new_push_messages() {
        ...
        let peer = ContactInfo::new_localhost(&aeko_sdk::pubkey::new_rand(), 0);
    ...
    fn test_process_prune() {
        let mut crds = Crds::default();
        let self_id = aeko_sdk::pubkey::new_rand();
        let push = CrdsGossipPush::default();
