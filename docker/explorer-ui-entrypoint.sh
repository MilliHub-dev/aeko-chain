...
# Canonical network-specific names. Legacy PUBLIC names remain fallback-only
# for the all-in-one rollback Compose.
AEKO_TESTNET_RPC_URL="${AEKO_TESTNET_RPC_URL:-${AEKO_PUBLIC_RPC_URL:-}}"
AEKO_TESTNET_WS_URL="${AEKO_TESTNET_WS_URL:-${AEKO_PUBLIC_WS_URL:-}}"
export AEKO_TESTNET_RPC_URL AEKO_TESTNET_WS_URL
...
const testnet = {
  rpcUrl: optionalAny('AEKO_TESTNET_RPC_URL', 'AEKO_PUBLIC_RPC_URL'),
  websocketUrl: optionalAny('AEKO_TESTNET_WS_URL', 'AEKO_PUBLIC_WS_URL'),
...
const mainnetExplorerUpstream = optionalAny(
...
const localnetUpstreamEnv = optionalAny(
