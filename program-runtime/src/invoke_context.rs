...
macro_rules! declare_process_instruction {
    ($process_instruction:ident, $cu_to_consume:expr, |$invoke_context:ident| $inner:tt) => {
        $crate::aeko_rbpf::declare_builtin_function!(
            $process_instruction,
            fn rust(
                ...
                _arg3: u64,
                _arg4: u64,
                _memory_mapping: &mut $crate::aeko_rbpf::memory_region::MemoryMapping,
            ) -> std::result::Result<u64, Box<dyn std::error::Error>> {
                fn process_instruction_inner(
                    $invoke_context: &mut $crate::invoke_context::InvokeContext,
                ) -> std::result::Result<(), aeko_sdk::instruction::InstructionError>
                    $inner
...
mod tests {
    use {
        ...
        aeko_sdk::{account::WritableAccount, instruction::Instruction, rent::Rent},
    };
    ...
    fn test_instruction_stack_height() {
        ...
        for index in 0..one_more_than_max_depth {
            invoke_stack.push(aeko_sdk::pubkey::new_rand());
        ...
        for (index, program_id) in invoke_stack.iter().enumerate() {
            transaction_accounts.push((
                *program_id,
                AccountSharedData::new(1, 1, &aeko_sdk::pubkey::Pubkey::default()),
            ));
            instruction_accounts.push(InstructionAccount {
                index_in_transaction: index as IndexOfAccount,
                index_in_caller: index as IndexOfAccount,
    ...
    fn test_process_instruction() {
        let callee_program_id = aeko_sdk::pubkey::new_rand();
        let owned_account = AccountSharedData::new(42, 1, &callee_program_id);
        let not_owned_account = AccountSharedData::new(84, 1, &aeko_sdk::pubkey::new_rand());
        let readonly_account = AccountSharedData::new(168, 1, &aeko_sdk::pubkey::new_rand());
        ...
        let transaction_accounts = vec![
            (aeko_sdk::pubkey::new_rand(), owned_account),
            (aeko_sdk::pubkey::new_rand(), not_owned_account),
            (aeko_sdk::pubkey::new_rand(), readonly_account),
            (callee_program_id, program_account),
            (aeko_sdk::pubkey::new_rand(), loader_account),
        ];
        let metas = vec![
    ...
    fn test_invoke_context_compute_budget() {
        let transaction_accounts =
            vec![(aeko_sdk::pubkey::new_rand(), AccountSharedData::default())];
