...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = aeko_perf::perf_libs::locate_perf_libs()
            {
                aeko_perf::perf_libs::append_to_ld_library_path(
...
/// * For TVU: `aeko_core::replay_stage::ReplayStage::replay_blockstore_into_bank()`
///
...
///
/// This enforcement is done via a call to `aeko_runtime::accounts::Accounts::lock_accounts()`
...
mod tests {
    use {
        super::*,
        aeko_perf::test_tx::{test_invalid_tx, test_tx},
        aeko_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
    ...
    fn test_transaction_signing() {
        use aeko_sdk::signature::Signature;
        let zero = Hash::default();
