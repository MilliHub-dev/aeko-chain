...
use {
    ...
    aeko_accounts_db::stake_rewards::StakeReward,
    aeko_sdk::{
    ...
    aeko_vote::vote_account::{VoteAccount, VoteAccounts},
    std::{
...
impl StakesCache {
    ...
    pub(crate) fn check_and_store(
        ...
        if account.lamports() == 0 {
            if aeko_vote_program::check_id(owner) {
                let mut stakes = self.0.write().unwrap();
                stakes.remove_vote_account(pubkey);
            } else if aeko_stake_program::check_id(owner) {
                let mut stakes = self.0.write().unwrap();
...
pub(crate) mod tests {
    use {
        ...
        aeko_sdk::{account::WritableAccount, pubkey::Pubkey, rent::Rent, stake},
    ...
    pub(crate) fn create_staked_node_accounts(
        ...
        let vote_account =
            vote_state::create_account(&vote_pubkey, &aeko_sdk::pubkey::new_rand(), 0, 1);
        let stake_pubkey = aeko_sdk::pubkey::new_rand();
        (
    ...
    fn test_stakes_basic() {
        for i in 0..4 {
            ...
            let mut stake_account =
                create_stake_account(42, &vote_pubkey, &aeko_sdk::pubkey::new_rand());
            stakes_cache.check_and_store(&stake_pubkey, &stake_account, None);
            let stake = stake_state::stake_from(&stake_account).unwrap();
            {
                let stakes = stakes_cache.stakes();
    ...
    fn test_stakes_multiple_stakers() {
        ...
        let stake_pubkey2 = aeko_sdk::pubkey::new_rand();
    ...
    fn test_serde_stakes_enum_compat() {
        ...
        for _ in 0..rng.gen_range(5usize..10) {
            ...
            let vote_account = vote_state::create_account(
                &vote_pubkey,
                &aeko_sdk::pubkey::new_rand(), // node_pubkey
