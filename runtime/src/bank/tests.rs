...
use {
    ...
    aeko_sdk::{
        ...
        native_loader,
        native_token::{aeko_to_lamports, LAMPORTS_PER_AEKO},
    ...
    aeko_stake_program::stake_state::{self, StakeStateV2},
    aeko_svm::transaction_results::DurableNonceFee,
    aeko_vote_program::{
        vote_instruction,
        vote_state::{
...
impl VoteReward {
    pub fn new_random() -> Self {
        ...
        let validator_pubkey = aeko_sdk::pubkey::new_rand();
...
fn create_genesis_config_no_tx_fee_no_rent(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let genesis_config_info = aeko_runtime::genesis_utils::create_genesis_config(lamports);
    (
...
fn create_genesis_config_no_tx_fee(lamports: u64) -> (GenesisConfig, Keypair) {
    ...
    let (mut genesis_config, mint_keypair) =
        aeko_sdk::genesis_config::create_genesis_config(lamports);
...
fn test_race_register_tick_freeze() {
    ...
    let p = aeko_sdk::pubkey::new_rand();
...
fn test_bank_new() {
    let dummy_leader_pubkey = aeko_sdk::pubkey::new_rand();
...
fn test_rent_distribution() {
    ...
    let bootstrap_validator_pubkey = aeko_sdk::pubkey::new_rand();
    ...
    let validator_1_pubkey = aeko_sdk::pubkey::new_rand();
    ...
    let validator_2_pubkey = aeko_sdk::pubkey::new_rand();
    ...
    let validator_3_pubkey = aeko_sdk::pubkey::new_rand();
...
fn test_rent_exempt_executable_account() {
    ...
    let account_pubkey = aeko_sdk::pubkey::new_rand();
    let account_balance = 1;
    let mut account = AccountSharedData::new(account_balance, 0, &aeko_sdk::pubkey::new_rand());
...
fn test_rent_eager_across_epoch_without_gap_under_multi_epoch_cycle() {
    let leader_pubkey = aeko_sdk::pubkey::new_rand();
...
fn test_rent_eager_collect_rent_in_partition(should_collect_rent: bool) {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != aeko_sdk::feature_set::skip_rent_rewrites::id()
            && (!should_collect_rent
                || feature_id != aeko_sdk::feature_set::disable_rent_fees_collection::id())
        {
            activate_feature(&mut genesis_config, feature_id);
        }
    }

    let zero_lamport_pubkey = aeko_sdk::pubkey::new_rand();
    let rent_due_pubkey = aeko_sdk::pubkey::new_rand();
    let rent_exempt_pubkey = aeko_sdk::pubkey::new_rand();
    let mut bank = Arc::new(Bank::new_for_tests(&genesis_config));
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let vote_id = aeko_sdk::pubkey::new_rand();
    let mut vote_account =
        vote_state::create_account(&vote_id, &aeko_sdk::pubkey::new_rand(), 0, 100);
    let stake_id1 = aeko_sdk::pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = aeko_sdk::pubkey::new_rand();
...
fn test_purge_empty_accounts() {
    ...
    for pass in 0..3 {
        aeko_logger::setup();
        let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(aeko_to_lamports(1.));
...
fn test_two_payments_to_one_party() {
    let (genesis_config, mint_keypair) = create_genesis_config(aeko_to_lamports(1.));
    let pubkey = aeko_sdk::pubkey::new_rand();
...
fn test_one_source_two_tx_one_batch() {
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(aeko_to_lamports(1.));
    let key1 = aeko_sdk::pubkey::new_rand();
    let key2 = aeko_sdk::pubkey::new_rand();
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
    let amount = genesis_config.rent.minimum_balance(0);
    assert_eq!(bank.last_blockhash(), genesis_config.hash());
...
fn test_insufficient_funds() {
    let mint_amount = aeko_to_lamports(1.);
...
fn test_transfer_to_sysvar() {
    ...
    let normal_pubkey = aeko_sdk::pubkey::new_rand();
...
fn test_bank_withdraw() {
    ...
    // Test no account
    let key = aeko_sdk::pubkey::new_rand();
    assert_eq!(
...
fn test_bank_tx_fee() {
    ...
    let leader = aeko_sdk::pubkey::new_rand();
    let GenesisConfigInfo {
        mut genesis_config,
        mint_keypair,
        ..
...
fn test_debits_before_credits() {
    let (genesis_config, mint_keypair) =
        create_genesis_config_no_tx_fee_no_rent(aeko_to_lamports(2.));
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
    let keypair = Keypair::new();
...
fn test_readonly_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &aeko_sdk::pubkey::new_rand(), 0);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;

    let vote_pubkey0 = aeko_sdk::pubkey::new_rand();
    let vote_pubkey1 = aeko_sdk::pubkey::new_rand();
    let vote_pubkey2 = aeko_sdk::pubkey::new_rand();
...
fn test_readonly_relaxed_locks() {
    ...
    let key3 = aeko_sdk::pubkey::new_rand();
...
fn test_bank_parent_account_spend() {
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(aeko_to_lamports(1.0));
    let key1 = Keypair::new();
    let key2 = Keypair::new();
    let (parent, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
...
fn test_bank_hash_internal_state() {
    let (genesis_config, mint_keypair) =
        create_genesis_config_no_tx_fee_no_rent(aeko_to_lamports(1.));
    ...
    let pubkey2 = aeko_sdk::pubkey::new_rand();
...
fn test_hash_internal_state_order() {
    ...
    assert_eq!(bank0.hash_internal_state(), bank1.hash_internal_state());
    let key0 = aeko_sdk::pubkey::new_rand();
    let key1 = aeko_sdk::pubkey::new_rand();
...
fn test_hash_internal_state_error() {
    ...
    assert!(bank
        .transfer(aeko_to_lamports(1.), &mint_keypair, &key0)
        .is_err());
...
fn test_bank_squash() {
    aeko_logger::setup();
    let (genesis_config, mint_keypair) = create_genesis_config_no_tx_fee(aeko_to_lamports(2.));
    let key1 = Keypair::new();
    let key2 = Keypair::new();
    let (parent, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let amount = genesis_config.rent.minimum_balance(0);
...
fn test_bank_update_sysvar_account() {
    ...
    for pass in 0..5 {
        ...
        let dummy_clock_id = aeko_sdk::pubkey::new_rand();
...
fn test_is_delta_true() {
    let (genesis_config, mint_keypair) = create_genesis_config(aeko_to_lamports(1.0));
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
    let key1 = Keypair::new();
...
fn test_bank_vote_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(500, &aeko_sdk::pubkey::new_rand(), 1);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
...
fn test_bank_cloned_stake_delegations() {
    ...
    let (vote_balance, stake_balance) = {
        ...
        let minimum_delegation = aeko_stake_program::get_minimum_delegation(&bank.feature_set);
        (
...
fn test_is_delta_with_no_committables() {
    ...
    assert_eq!(
        bank.transfer(10_001, &mint_keypair, &aeko_sdk::pubkey::new_rand()),
        Err(TransactionError::InstructionError(
            0,
            SystemError::ResultWithNegativeLamports.into(),
        ))
...
fn test_bank_get_program_accounts() {
    ...
    assert!(
        genesis_accounts
            .iter()
            .any(|(pubkey, _, _)| aeko_sdk::sysvar::is_sysvar_id(pubkey)),
        "no sysvars found"
    );
...
fn test_add_duplicate_static_program() {
    ...
    bank.add_mockup_builtin(aeko_vote_program::id(), MockBuiltin::vm);
    let bank = bank_forks
        .write()
        .unwrap()
        .insert(bank)
    ...
    let vote_loader_account = bank.get_account(&aeko_vote_program::id()).unwrap();
    let new_vote_loader_account = bank.get_account(&aeko_vote_program::id()).unwrap();
...
fn test_transaction_with_duplicate_accounts_in_instruction() {
    ...
    let from_pubkey = aeko_sdk::pubkey::new_rand();
    let to_pubkey = aeko_sdk::pubkey::new_rand();
    let dup_pubkey = from_pubkey;
    let from_account = AccountSharedData::new(aeko_to_lamports(100.), 1, &mock_program_id);
    ...
    let instruction =
        Instruction::new_with_bincode(mock_program_id, &aeko_to_lamports(10.), account_metas);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    assert_eq!(bank.get_balance(&from_pubkey), aeko_to_lamports(80.));
    assert_eq!(bank.get_balance(&to_pubkey), aeko_to_lamports(20.));
}
...
fn test_account_ids_after_program_ids() {
    ...
    let instruction = Instruction::new_with_bincode(aeko_vote_program::id(), &10, account_metas);
    let mut tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    tx.message.account_keys.push(aeko_sdk::pubkey::new_rand());
    ...
    let result = bank.process_transaction(&tx);
    assert_eq!(result, Ok(()));
    let account = bank.get_account(&aeko_vote_program::id()).unwrap();
...
fn test_program_id_as_payer() {
    ...
    tx.message.account_keys.push(aeko_vote_program::id());
...
fn test_bank_hash_consistency() {
    ...
    activate_feature(
        &mut genesis_config,
        aeko_sdk::feature_set::set_exempt_rent_epoch_max::id(),
    );
...
fn test_add_builtin_no_overwrite() {
    let slot = 123;
    let program_id = aeko_sdk::pubkey::new_rand();
...
fn test_add_builtin_loader_no_overwrite() {
    let slot = 123;
    let loader_id = aeko_sdk::pubkey::new_rand();
...
fn test_reconfigure_token2_native_mint() {
    ...
    let genesis_config =
        create_genesis_config_with_leader(5, &aeko_sdk::pubkey::new_rand(), 0).genesis_config;
    let bank = Arc::new(Bank::new_for_tests(&genesis_config));
    assert_eq!(
...
fn test_bank_load_program() {
    ...
    let programdata_key = aeko_sdk::pubkey::new_rand();
...
fn test_bpf_loader_upgradeable_deploy_with_max_len() {
    ...
    let payer_base_balance = LAMPORTS_PER_AEKO;
    let deploy_fees = {
    ...
    mock_process_instruction(
        ...
        Vec::new(),
        Ok(()),
        aeko_bpf_loader_program::Entrypoint::vm,
        |invoke_context| {
            invoke_context
        ...
        ) + std::marker::Send,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &aeko_sdk::pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
    ...
    let all_pubkeys: Vec<Pubkey> = std::iter::repeat_with(aeko_sdk::pubkey::new_rand)
        .take(total_pubkeys)
        .collect();
...
fn test_get_inflation_start_slot_devnet_testnet() {
    let GenesisConfigInfo {
        mut genesis_config, ..
    } = create_genesis_config_with_leader(42, &aeko_sdk::pubkey::new_rand(), 42);
    genesis_config
        .accounts
...
fn test_get_largest_accounts() {
    ...
    let pubkeys_balances: Vec<_> = pubkeys
        ...
        .cloned()
        .zip(vec![
            aeko_to_lamports(2.0),
            aeko_to_lamports(3.0),
            aeko_to_lamports(3.0),
            aeko_to_lamports(4.0),
            aeko_to_lamports(5.0),
        ])
        .collect();
    ...
    assert_eq!(
        bank.get_largest_accounts(1, &pubkeys_hashset, AccountAddressFilter::Include)
            .unwrap(),
        vec![(pubkeys[4], aeko_to_lamports(5.0))]
    );
    assert_eq!(
    ...
    assert_eq!(
        bank.get_largest_accounts(1, &exclude4, AccountAddressFilter::Exclude)
            .unwrap(),
        vec![(pubkeys[3], aeko_to_lamports(4.0))]
    );
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        aeko_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    let message_arr = b"hello";
    let instruction =
        aeko_sdk::ed25519_instruction::new_ed25519_instruction(&privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_an_empty_instruction_without_program() {
    ...
    let destination = aeko_sdk::pubkey::new_rand();
...
fn test_accounts_data_size_with_good_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000 * LAMPORTS_PER_AEKO);
    let bank = Bank::new_for_tests(&genesis_config);
    let bank = bank.wrap_with_bank_forks_for_tests().0;
...
fn test_accounts_data_size_with_bad_transaction() {
    const ACCOUNT_SIZE: u64 = MAX_PERMITTED_DATA_LENGTH;
    let (genesis_config, _mint_keypair) = create_genesis_config(1_000 * LAMPORTS_PER_AEKO);
    let bank = Bank::new_for_tests(&genesis_config);
    let bank = bank.wrap_with_bank_forks_for_tests().0;
    let transaction = system_transaction::create_account(
        &Keypair::new(),
...
fn test_invalid_rent_state_changes_existing_accounts() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = create_genesis_config_with_leader(aeko_to_lamports(100.), &Pubkey::new_unique(), 42);
    genesis_config.rent = Rent::default();
...
fn test_rent_state_changes_sysvars() {
    ...
    let validator_pubkey = aeko_sdk::pubkey::new_rand();
    let validator_stake_lamports = aeko_to_lamports(1.);
...
fn test_invalid_rent_state_changes_fee_payer() {
    ...
    genesis_config.fee_rate_governor = FeeRateGovernor::new(
        aeko_sdk::fee_calculator::DEFAULT_TARGET_LAMPORTS_PER_SIGNATURE,
        aeko_sdk::fee_calculator::DEFAULT_TARGET_SIGNATURES_PER_SLOT,
    );
...
fn test_rent_state_incinerator() {
    ...
    for amount in [rent_exempt_minimum - 1, rent_exempt_minimum] {
        bank.transfer(amount, &mint_keypair, &aeko_sdk::incinerator::id())
            .unwrap();
    }
}
...
fn test_resize_and_rent() {
    ...
    let rent_paying_pubkey = aeko_sdk::pubkey::new_rand();
...
fn test_accounts_data_size_and_resize_transactions() {
    let GenesisConfigInfo {
        ...
        mint_keypair,
        ..
    } = genesis_utils::create_genesis_config(100 * LAMPORTS_PER_AEKO);
    let mock_program_id = Pubkey::new_unique();
    let bank = Bank::new_with_mockup_builtin_for_tests(
        &genesis_config,
        mock_program_id,
    ...
    bank.store_account(
        &funding_keypair.pubkey(),
        &AccountSharedData::new(10 * LAMPORTS_PER_AEKO, 0, &mock_program_id),
    );
    ...
    // Test case: Grow account
    {
        ...
        let account_balance = LAMPORTS_PER_AEKO;
        let account_size =
...
fn test_accounts_data_size_and_rent_collection(should_collect_rent: bool) {
    ...
    if should_collect_rent {
        genesis_config
            .accounts
            .remove(&aeko_sdk::feature_set::disable_rent_fees_collection::id());
    }
...
fn test_accounts_data_size_from_genesis() {
    let GenesisConfigInfo {
        ...
        1_000_000 * LAMPORTS_PER_AEKO,
        &Pubkey::new_unique(),
        100 * LAMPORTS_PER_AEKO,
    );
    genesis_config.rent = Rent::default();
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_AEKO);
    let bank = Bank::new_with_bank_forks_for_tests(&genesis_config).0;
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            aeko_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_is_in_slot_hashes_history() {
    use aeko_sdk::slot_hashes::MAX_ENTRIES;
...
fn test_feature_activation_loaded_programs_recompilation_phase() {
    ...
    // Bank Setup
    let (mut genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_AEKO);
    genesis_config
        .accounts
...
fn test_rewards_point_calculation_empty() {
    ...
    let (genesis_config, _mint_keypair) = create_genesis_config(aeko_to_lamports(1.0));
    let bank = Bank::new_for_tests(&genesis_config);
...
fn test_force_reward_interval_end() {
    let (genesis_config, _mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_AEKO);
    let mut bank = Bank::new_for_tests(&genesis_config);
...
fn test_get_stake_rewards_partition_range_panic() {
    let (mut genesis_config, _mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_AEKO);
...
fn test_program_execution_restricted_for_stake_account_in_reward_period() {
    use aeko_sdk::transaction::TransactionError::ProgramExecutionTemporarilyRestricted;
    ...
    F: Fn(&Bank),
{
    ...
    let mint_lamports = aeko_to_lamports(1.0);
    let len1 = 123;
    let len2 = 456;
...
fn test_system_instruction_unsigned_transaction() {
    let (genesis_config, alice_keypair) = create_genesis_config_no_tx_fee(aeko_to_lamports(1.0));
    let alice_pubkey = alice_keypair.pubkey();
...
fn test_calc_vote_accounts_to_store_three() {
    ...
    let pubkey = aeko_sdk::pubkey::new_rand();
    let pubkey2 = aeko_sdk::pubkey::new_rand();
    let pubkey3 = aeko_sdk::pubkey::new_rand();
    let mut vote_account = AccountSharedData::default();
    vote_account.set_lamports(u64::MAX);
    vote_account_rewards.insert(
        pubkey,
...
fn test_failed_simulation_compute_units() {
    let (genesis_config, mint_keypair) = create_genesis_config(LAMPORTS_PER_AEKO);
    let program_id = Pubkey::new_unique();
    let bank =
        Bank::new_with_mockup_builtin_for_tests(&genesis_config, program_id, MockBuiltin::vm).0;
