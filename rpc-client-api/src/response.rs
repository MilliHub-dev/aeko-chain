use {
    ...
    aeko_account_decoder::{parse_token::UiTokenAmount, UiAccount},
    aeko_sdk::{
...
impl Default for RpcApiVersion {
    fn default() -> Self {
        Self(aeko_version::Version::default().as_semver_version())
    }
}
...
pub struct RpcVersionInfo {
    /// The current version of aeko-core
    pub aeko_core: String,
    /// first 4 bytes of the FeatureSet identifier
    pub feature_set: Option<u32>,
}
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.aeko_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.aeko_core.split_whitespace().next() {
