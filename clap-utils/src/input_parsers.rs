...
pub fn lamports_of_aeko(matches: &ArgMatches<'_>, name: &str) -> Option<u64> {
...
mod tests {
    use {
        super::*,
        clap::{App, Arg},
        aeko_sdk::signature::write_keypair_file,
        std::fs,
    };
    ...
    #[test]
    fn test_lamports_of_aeko() {
        let matches = app().get_matches_from(vec!["test", "--single", "50"]);
        assert_eq!(lamports_of_aeko(&matches, "single"), Some(50_000_000_000));
        assert_eq!(lamports_of_aeko(&matches, "multiple"), None);
        let matches = app().get_matches_from(vec!["test", "--single", "1.5"]);
        assert_eq!(lamports_of_aeko(&matches, "single"), Some(1_500_000_000));
        assert_eq!(lamports_of_aeko(&matches, "multiple"), None);
        let matches = app().get_matches_from(vec!["test", "--single", "0.03"]);
        assert_eq!(lamports_of_aeko(&matches, "single"), Some(30_000_000));
    }
}
