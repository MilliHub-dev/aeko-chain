    ...
    pub fn finalize(self) -> Hash {
        ...
        if let Ok(dir) = std::env::var("AEKO_ABI_DUMP_DIR") {
            let thread_name = std::thread::current()
                ...
                .replace(':', "_");
            if thread_name == "main" {
                error!("Bad thread name detected for dumping; Maybe, --test-threads=1? Sorry, AEKO_ABI_DUMP_DIR doesn't work under 1; increase it");
            }
...
mod tests {
    ...
    mod bitflags_abi {
        ...
        //   aeko_frozen_abi::abi_digester::tests::_::InternalBitFlags
