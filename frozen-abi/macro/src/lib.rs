...
fn derive_abi_sample_enum_type(input: ItemEnum) -> TokenStream {
    ...
    let result = quote! {
        ...
        #( #attrs )*
        impl #impl_generics ::aeko_frozen_abi::abi_example::AbiExample for #type_name #ty_generics #where_clause {
            fn example() -> Self {
                ::log::info!(
                    "AbiExample for enum: {}",
...
fn do_derive_abi_enum_visitor(input: ItemEnum) -> TokenStream {
    ...
    (quote! {
        impl #impl_generics ::aeko_frozen_abi::abi_example::AbiEnumVisitor for #type_name #ty_generics #where_clause {
            fn visit_for_abi(&self, digester: &mut ::aeko_frozen_abi::abi_digester::AbiDigester) -> ::aeko_frozen_abi::abi_digester::DigestResult {
...
fn quote_for_test(
    ...
    quote! {
        ...
        mod #test_mod_ident {
            use super::*;
            use ::aeko_frozen_abi::abi_example::{AbiExample, AbiEnumVisitor};

            #[test]
            fn test_abi_digest() {
                ::aeko_logger::setup();
                let mut digester = ::aeko_frozen_abi::abi_digester::AbiDigester::create();
                ...
                if ::std::env::var("AEKO_ABI_BULK_UPDATE").is_ok() {
                    if #expected_digest != actual_digest {
                        #p!("sed -i -e 's/{}/{}/g' $(git grep --files-with-matches frozen_abi)", #expected_digest, hash);
                    }
                    ::log::warn!("Not testing the abi digest under AEKO_ABI_BULK_UPDATE!");
                } else {
                    if let Ok(dir) = ::std::env::var("AEKO_ABI_DUMP_DIR") {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Examine the diff in AEKO_ABI_DUMP_DIR!: \n$ diff -u {}/*{}* {}/*{}*", dir, #expected_digest, dir, actual_digest);
                    } else {
                        assert_eq!(#expected_digest, actual_digest, "Possibly ABI changed? Confirm the diff by rerunning before and after this test failed with AEKO_ABI_DUMP_DIR!");
                    }
                }
            }
        }
