...
#[deprecated(
    since = "1.8.0",
    note = "Please use `aeko_sdk::stake::state` or `aeko_program::stake::state` instead"
)]
pub use aeko_sdk::stake::state::*;
use {
...
pub fn delegate(
    ...
    if *vote_account.get_owner() != aeko_vote_program::id() {
...
pub(crate) fn deactivate_delinquent(
    ...
    if *delinquent_vote_account.get_owner() != aeko_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
    ...
    if *reference_vote_account.get_owner() != aeko_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
...
mod tests {
    use {
        super::*,
        proptest::prelude::*,
        aeko_program_runtime::with_mock_invoke_context,
        aeko_sdk::{
    ...
    fn test_authorized_authorize() {
        let staker = aeko_sdk::pubkey::new_rand();
    ...
    fn test_authorized_authorize_with_custodian() {
        ...
        let invalid_custodian = aeko_sdk::pubkey::new_rand();
        let mut authorized = Authorized::auto(&staker);
        let mut signers = HashSet::new();
        signers.insert(staker);
    ...
    fn test_dbg_stake_minimum_balance() {
        ...
        panic!(
            "stake minimum_balance: {} lamports, {} AEKO",
            minimum_balance,
            minimum_balance as f64 / aeko_sdk::native_token::LAMPORTS_PER_AEKO as f64
        );
    }
