...
use {
    crate::vote_state,
    log::*,
    aeko_program::vote::{instruction::VoteInstruction, program::id, state::VoteAuthorize},
...
mod tests {
    ...
    fn create_test_account() -> (Pubkey, AccountSharedData) {
        ...
        (
            vote_pubkey,
            vote_state::create_account(&vote_pubkey, &aeko_sdk::pubkey::new_rand(), 0, balance),
        )
    }

    fn create_test_account_with_authorized() -> (Pubkey, Pubkey, Pubkey, AccountSharedData) {
        let vote_pubkey = aeko_sdk::pubkey::new_rand();
        let authorized_voter = aeko_sdk::pubkey::new_rand();
        let authorized_withdrawer = aeko_sdk::pubkey::new_rand();
    ...
    fn test_authorize_voter() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_voter_pubkey = aeko_sdk::pubkey::new_rand();
        let clock = Clock {
            epoch: 1,
    ...
    fn test_authorize_withdrawer() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_withdrawer_pubkey = aeko_sdk::pubkey::new_rand();
        let instruction_data = serialize(&VoteInstruction::Authorize(
