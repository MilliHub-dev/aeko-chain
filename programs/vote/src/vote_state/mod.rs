...
pub use aeko_program::vote::state::{vote_state_versions::*, *};
use {
    log::*,
    serde_derive::{Deserialize, Serialize},
    aeko_metrics::datapoint_debug,
    aeko_program::vote::{error::VoteError, program::id},
    aeko_sdk::{
    ...
    for vote in &new_state {
        if vote.confirmation_count() == 0 {
            ...
            if vote.slot() <= new_root
                ...
                // https://github.com/ryoqun/aeko/blob/df55bfb46af039cbc597cd60042d49b9d90b5961/core/src/consensus.rs#L120
                // always sets a root for even empty towers, which is then hard unwrapped here
                // https://github.com/ryoqun/aeko/blob/df55bfb46af039cbc597cd60042d49b9d90b5961/core/src/consensus.rs#L776
...
mod tests {
    ...
    fn vote_state_new_for_test(auth_pubkey: &Pubkey) -> VoteState {
        VoteState::new(
            &VoteInit {
                node_pubkey: aeko_sdk::pubkey::new_rand(),
    ...
    fn create_test_account() -> (Pubkey, RefCell<AccountSharedData>) {
        ...
        let vote_pubkey = aeko_sdk::pubkey::new_rand();
        (
            vote_pubkey,
    ...
    fn test_vote_state_upgrade_from_1_14_11() {
        ...
        let node_pubkey = aeko_sdk::pubkey::new_rand();
        let withdrawer_pubkey = aeko_sdk::pubkey::new_rand();
        ...
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&aeko_sdk::pubkey::new_rand(), 0, 1, |_pubkey| Ok(())),
            Ok(())
        );
        vote_state.increment_credits(1, 200);
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&aeko_sdk::pubkey::new_rand(), 1, 2, |_pubkey| Ok(())),
            Ok(())
        );
        vote_state.increment_credits(2, 300);
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&aeko_sdk::pubkey::new_rand(), 2, 3, |_pubkey| Ok(())),
            Ok(())
        );
        ...
        let processor_account = AccountSharedData::new(0, 0, &aeko_sdk::native_loader::id());
    ...
    fn test_vote_double_lockout_after_expiration() {
        let voter_pubkey = aeko_sdk::pubkey::new_rand();
    ...
    fn test_process_missed_votes() {
        let account_a = aeko_sdk::pubkey::new_rand();
        let mut vote_state_a = vote_state_new_for_test(&account_a);
        let account_b = aeko_sdk::pubkey::new_rand();
