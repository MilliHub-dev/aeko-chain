...
use {
    itertools::izip,
    aeko_account_decoder::parse_bpf_loader::{
        parse_bpf_upgradeable_loader, BpfUpgradeableLoaderAccountType,
    },
    aeko_ledger::token_balances::collect_token_balances,
    ...
    aeko_sbf_rust_invoke::instructions::*,
    aeko_sbf_rust_realloc::instructions::*,
    aeko_sbf_rust_realloc_invoke::instructions::*,
    aeko_sdk::{
    ...
    aeko_svm::transaction_processor::ExecutionRecordingConfig,
    aeko_svm::transaction_results::{
...
fn test_program_sbf_sanity() {
    ...
    {
        programs.extend_from_slice(&[
            ("aeko_sbf_rust_128bit", true),
            ("aeko_sbf_rust_alloc", true),
            ("aeko_sbf_rust_alt_bn128", true),
            ("aeko_sbf_rust_alt_bn128_compression", true),
            ("aeko_sbf_rust_curve25519", true),
            ("aeko_sbf_rust_custom_heap", true),
            ("aeko_sbf_rust_dep_crate", true),
            ("aeko_sbf_rust_external_spend", false),
            ("aeko_sbf_rust_iter", true),
            ("aeko_sbf_rust_many_args", true),
            ("aeko_sbf_rust_membuiltins", true),
            ("aeko_sbf_rust_noop", true),
            ("aeko_sbf_rust_panic", false),
            ("aeko_sbf_rust_param_passing", true),
            ("aeko_sbf_rust_poseidon", true),
            ("aeko_sbf_rust_rand", true),
            ("aeko_sbf_rust_sanity", true),
            ("aeko_sbf_rust_secp256k1_recover", true),
            ("aeko_sbf_rust_sha", true),
        ]);
    }
...
fn test_program_sbf_loader_deprecated() {
    ...
    #[cfg(feature = "sbf_rust")]
    {
        programs.extend_from_slice(&[("aeko_sbf_rust_deprecated_loader")]);
    }
    ...
    for program in programs.iter() {
        ...
        genesis_config
            .accounts
            .remove(&aeko_sdk::feature_set::disable_deploy_of_alloc_free_syscall::id())
            .unwrap();
        let (bank, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
...
fn test_sol_alloc_free_no_longer_deployable_with_upgradeable_loader() {
    ...
    // Populate loader account with `aeko_sbf_rust_deprecated_loader` elf, which
    // depends on `sol_alloc_free_` syscall. This can be verified with
    // $ elfdump aeko_sbf_rust_deprecated_loader.so
    ...
    let (_bank, _program_id) = load_upgradeable_program_and_advance_slot(
        ...
        &mint_keypair,
        &authority_keypair,
        "aeko_sbf_rust_deprecated_loader",
    );
}
...
fn test_program_sbf_duplicate_accounts() {
    ...
    #[cfg(feature = "sbf_rust")]
    {
        programs.extend_from_slice(&[("aeko_sbf_rust_dup_accounts")]);
    }
...
fn test_program_sbf_error_handling() {
    ...
    #[cfg(feature = "sbf_rust")]
    {
        programs.extend_from_slice(&[("aeko_sbf_rust_error_handling")]);
    }
...
fn test_return_data_and_log_data_syscall() {
    ...
    #[cfg(feature = "sbf_rust")]
    {
        programs.extend_from_slice(&[("aeko_sbf_rust_log_data")]);
    }
...
fn test_program_sbf_invoke_sanity() {
    ...
    #[cfg(feature = "sbf_rust")]
    {
        programs.push((
            Languages::Rust,
            "aeko_sbf_rust_invoke",
            "aeko_sbf_rust_invoked",
            "aeko_sbf_rust_noop",
        ));
    }
    for program in programs.iter() {
        ...
        let account_metas = vec![
            ...
            AccountMeta::new_readonly(aeko_sdk::ed25519_program::id(), false),
...
fn assert_instruction_count() {
    ...
    {
        programs.extend_from_slice(&[
            ("aeko_sbf_rust_128bit", 1218),
            ("aeko_sbf_rust_alloc", 5077),
            ("aeko_sbf_rust_custom_heap", 398),
            ("aeko_sbf_rust_dep_crate", 2),
            ("aeko_sbf_rust_iter", 1514),
            ("aeko_sbf_rust_many_args", 1289),
            ("aeko_sbf_rust_mem", 2067),
            ("aeko_sbf_rust_membuiltins", 1539),
            ("aeko_sbf_rust_noop", 275),
            ("aeko_sbf_rust_param_passing", 146),
            ("aeko_sbf_rust_rand", 378),
            ("aeko_sbf_rust_sanity", 51953),
            ("aeko_sbf_rust_secp256k1_recover", 91185),
            ("aeko_sbf_rust_sha", 24059),
        ]);
    }
        ...
        mock_process_instruction(
            ...
            |invoke_context| {
                *prev_compute_meter.borrow_mut() = invoke_context.get_remaining();
                aeko_bpf_loader_program::test_utils::load_all_invoked_programs(invoke_context);
            },
            |invoke_context| {
...
fn test_program_sbf_instruction_introspection() {
    ...
    let (bank, program_id) = load_upgradeable_program_and_advance_slot(
        ...
        &mint_keypair,
        &authority_keypair,
        "aeko_sbf_rust_instruction_introspection",
    );
...
fn test_program_sbf_invoke_stable_genesis_and_bank() {
    ...
    load_upgradeable_program(
        ...
        &authority_keypair,
        "aeko_sbf_rust_invoke_and_return",
    );
...
fn test_program_sbf_disguised_as_sbf_loader() {
    ...
    #[cfg(feature = "sbf_rust")]
    {
        programs.extend_from_slice(&[("aeko_sbf_rust_noop")]);
    }
    ...
    for program in programs.iter() {
        ...
        bank.deactivate_feature(
            &aeko_sdk::feature_set::remove_bpf_loader_incorrect_program_id::id(),
        );
...
fn test_program_sbf_ro_account_modify() {
    ...
    let (bank, program_id) = load_upgradeable_program_and_advance_slot(
        ...
        &mint_keypair,
        &authority_keypair,
        "aeko_sbf_rust_ro_account_modify",
    );
...
fn test_program_sbf_realloc() {
    ...
    for direct_mapping in [false, true] {
        ...
        assert_eq!(&aeko_sdk::system_program::id(), account.owner());
        let data = bank_client.get_account_data(&pubkey).unwrap().unwrap();
                    ...
                    Message::new(
                        &[Instruction::new_with_bytes(
                            ...
                            vec![
                                ...
                                AccountMeta::new(aeko_sdk::system_program::id(), false),
                            ],
                        )],
                        Some(&mint_pubkey),
                    )
                ...
                Message::new(
                    &[Instruction::new_with_bytes(
                        ...
                        vec![
                            AccountMeta::new(pubkey, true),
                            AccountMeta::new_readonly(realloc_program_id, false),
                            AccountMeta::new_readonly(aeko_sdk::system_program::id(), false),
                        ],
                    )],
                    Some(&mint_pubkey),
                )
...
fn test_program_sbf_processed_inner_instruction() {
    ...
    let sibling_program_id = load_upgradeable_program_wrapper(
        ...
        &mint_keypair,
        &authority_keypair,
        "aeko_sbf_rust_sibling_instructions",
    );
    let sibling_inner_program_id = load_upgradeable_program_wrapper(
        &bank_client,
        &mint_keypair,
        &authority_keypair,
        "aeko_sbf_rust_sibling_inner_instructions",
    );
...
fn test_get_minimum_delegation() {
    ...
    let (_bank, program_id) = load_upgradeable_program_and_advance_slot(
        ...
        &mint_keypair,
        &authority_keypair,
        "aeko_sbf_rust_get_minimum_delegation",
    );
...
fn test_program_sbf_inner_instruction_alignment_checks() {
    ...
    let (bank, bank_forks) = Bank::new_with_bank_forks_for_tests(&genesis_config);
    let noop = create_program(&bank, &bpf_loader_deprecated::id(), "aeko_sbf_rust_noop");
    let inner_instruction_alignment_check = create_program(
        &bank,
        &bpf_loader_deprecated::id(),
        "aeko_sbf_rust_inner_instruction_alignment_check",
    );
...
fn test_cpi_change_account_data_memory_allocation() {
    use aeko_program_runtime::{declare_process_instruction, loaded_programs::LoadedProgram};
