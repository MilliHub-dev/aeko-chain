...
use aeko_program::{custom_heap_default, custom_panic_default, msg};

fn test_sha256_hasher() {
    use aeko_program::hash::{hashv, Hasher};
...
fn test_keccak256_hasher() {
    use aeko_program::keccak::{hashv, Hasher};
    let vals = &["Gaggablaghblagh!".as_ref(), "flurbos".as_ref()];
    let mut hasher = Hasher::default();
    hasher.hashv(vals);
    assert_eq!(hashv(vals), hasher.result());
