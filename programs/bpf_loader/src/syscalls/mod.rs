...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use aeko_zk_token_sdk::curve25519::{curve_syscall_traits::*, edwards, ristretto};
        match curve_id {
            CURVE25519_EDWARDS => {
                let cost = invoke_context
                    .get_compute_budget()
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use aeko_zk_token_sdk::curve25519::{
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use aeko_sdk::alt_bn128::prelude::{ALT_BN128_ADD, ALT_BN128_MUL, ALT_BN128_PAIRING};
        let budget = invoke_context.get_compute_budget();
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use aeko_rbpf::vm::ContextObject;
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        use aeko_sdk::alt_bn128::compression::prelude::{
...
mod tests {
    ...
    use {
        ...
        core::slice,
        aeko_program_runtime::{invoke_context::InvokeContext, with_mock_invoke_context},
        aeko_rbpf::{
    ...
    fn test_translate_type() {
        ...
        let instruction = Instruction::new_with_bincode(
            ...
            &"foobar",
            vec![AccountMeta::new(aeko_sdk::pubkey::new_rand(), false)],
        );
    ...
    fn test_translate_slice() {
        ...
        *data.first_mut().unwrap() = 10;
        ...
        // Pubkeys
        let mut data = vec![aeko_sdk::pubkey::new_rand(); 5];
        let memory_mapping = MemoryMapping::new(
            vec![MemoryRegion::new_readonly(
                unsafe {
        ...
        let translated_data =
            translate_slice::<Pubkey>(&memory_mapping, 0x100000000, data.len() as u64, true)
                .unwrap();
        assert_eq!(data, translated_data);
        *data.first_mut().unwrap() = aeko_sdk::pubkey::new_rand(); // Both should point to same place
        assert_eq!(data, translated_data);
    }
    ...
    fn test_syscall_sol_alloc_free() {
        // large alloc
        {
            ...
            let result = SyscallAllocFree::rust(
                invoke_context,
                aeko_sdk::entrypoint::HEAP_LENGTH as u64,
                0,
                0,
                0,
                0,
    ...
    fn test_syscall_edwards_curve_point_validation() {
        use aeko_zk_token_sdk::curve25519::curve_syscall_traits::CURVE25519_EDWARDS;
    ...
    fn test_syscall_ristretto_curve_point_validation() {
        use aeko_zk_token_sdk::curve25519::curve_syscall_traits::CURVE25519_RISTRETTO;
    ...
    fn test_syscall_edwards_curve_group_ops() {
        use aeko_zk_token_sdk::curve25519::curve_syscall_traits::{
    ...
    fn test_create_program_address() {
        // These tests duplicate the direct tests in aeko_program::pubkey
