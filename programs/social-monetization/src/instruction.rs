...
pub enum SocialMonetizationInstruction {
    InitializeConfig { state: SocialMonetizationStateAccount },
    SendCreatorTip { record: CreatorTipRecord },
    CreateSubscription { record: SubscriptionRecord },
    RenewSubscription { subscription_id: [u8; 32], valid_until_unix: i64 },
    CancelSubscription { subscription_id: [u8; 32] },
    UnlockPaidContent { record: PaidContentUnlockRecord },
    ClaimMonetizationPayout { creator: Pubkey, amount: u64 },
    ReadMonetizationState { creator: Option<Pubkey> },
    // Appended to preserve the Borsh discriminants of all deployed instructions.
    UpdateTreasury { treasury: Pubkey },
}

pub fn initialize_config(program_id: &Pubkey, state_pubkey: &Pubkey, payer_pubkey: &Pubkey, authority_pubkey: &Pubkey, state: SocialMonetizationStateAccount) -> Instruction {
    Instruction::new_with_borsh(*program_id, &SocialMonetizationInstruction::InitializeConfig { state }, vec![
...
pub fn cancel_subscription(program_id: &Pubkey, state_pubkey: &Pubkey, subscriber_pubkey: &Pubkey, subscription_id: [u8; 32]) -> Instruction {
    Instruction::new_with_borsh(*program_id, &SocialMonetizationInstruction::CancelSubscription { subscription_id }, vec![
        AccountMeta::new(*state_pubkey, false), AccountMeta::new_readonly(*subscriber_pubkey, true),
    ])
}
...
pub fn read_monetization_state(program_id: &Pubkey, state_pubkey: &Pubkey, creator: Option<Pubkey>) -> Instruction {
    Instruction::new_with_borsh(*program_id, &SocialMonetizationInstruction::ReadMonetizationState { creator }, vec![AccountMeta::new_readonly(*state_pubkey, false)])
}

pub fn update_treasury(program_id: &Pubkey, state_pubkey: &Pubkey, authority_pubkey: &Pubkey, treasury_pubkey: &Pubkey) -> Instruction {
    Instruction::new_with_borsh(*program_id, &SocialMonetizationInstruction::UpdateTreasury { treasury: *treasury_pubkey }, vec![
        AccountMeta::new(*state_pubkey, false),
        AccountMeta::new_readonly(*authority_pubkey, true),
