use {
    crate::{
        ...
        state::{CreatorRevenueAccount, SocialMonetizationStateAccount, SubscriptionState},
    },
...
impl Processor {
    pub fn process(invoke_context: &mut InvokeContext) -> Result<(), InstructionError> {
        let instruction = {
            ...
            SocialMonetizationInstruction::try_from_slice(instruction_context.get_instruction_data())
                .map_err(|_| InstructionError::InvalidInstructionData)?
        };
        match instruction {
            SocialMonetizationInstruction::InitializeConfig { state } => Self::process_initialize(invoke_context, state),
            SocialMonetizationInstruction::SendCreatorTip { record } => Self::process_send_tip(invoke_context, record),
            SocialMonetizationInstruction::CreateSubscription { record } => Self::process_create_subscription(invoke_context, record),
            SocialMonetizationInstruction::RenewSubscription { subscription_id, valid_until_unix } => Self::process_renew_subscription(invoke_context, subscription_id, valid_until_unix),
            SocialMonetizationInstruction::CancelSubscription { subscription_id } => Self::process_cancel_subscription(invoke_context, subscription_id),
            SocialMonetizationInstruction::UnlockPaidContent { record } => Self::process_unlock_paid_content(invoke_context, record),
            SocialMonetizationInstruction::ClaimMonetizationPayout { creator, amount } => Self::process_claim(invoke_context, creator, amount),
            SocialMonetizationInstruction::ReadMonetizationState { creator } => Self::process_read(invoke_context, creator),
            SocialMonetizationInstruction::UpdateTreasury { treasury } => Self::process_update_treasury(invoke_context, treasury),
        }
    }

    fn process_initialize(invoke_context: &mut InvokeContext, state: SocialMonetizationStateAccount) -> Result<(), InstructionError> {
        let transaction_context = &invoke_context.transaction_context;
        let instruction_context = transaction_context.get_current_instruction_context()?;
    ...
    fn process_update_treasury(invoke_context: &mut InvokeContext, treasury: Pubkey) -> Result<(), InstructionError> {
        let transaction_context = &invoke_context.transaction_context;
        let instruction_context = transaction_context.get_current_instruction_context()?;
        instruction_context.check_number_of_instruction_accounts(3)?;
        ...
        let mut state_account = instruction_context.try_borrow_instruction_account(transaction_context, 0)?;
        if *state_account.get_owner() != crate::id() { return Err(InstructionError::InvalidAccountOwner); }
        let mut state = SocialMonetizationStateAccount::deserialize_padded(state_account.get_data()).map_err(|_| InstructionError::InvalidAccountData)?;
    ...
    fn process_send_tip(invoke_context: &mut InvokeContext, record: crate::state::CreatorTipRecord) -> Result<(), InstructionError> {
        let sender_key = Self::signer_key(invoke_context, 1)?;
        if sender_key != record.sender { return Err(InstructionError::IncorrectAuthority); }
        let treasury = {
            let transaction_context = &invoke_context.transaction_context;
            let instruction_context = transaction_context.get_current_instruction_context()?;
            instruction_context.check_number_of_instruction_accounts(3)?;
            ...
            let mut state = SocialMonetizationStateAccount::deserialize_padded(state_account.get_data()).map_err(|_| InstructionError::InvalidAccountData)?;
            state.ensure_initialized().map_err(Self::map_program_error)?;
            if record.amount == 0 { return Err(Self::map_program_error(SocialMonetizationError::InvalidAmount.into())); }
            if state.tip_exists(&record.tip_id) { return Err(Self::map_program_error(SocialMonetizationError::DuplicateTip.into())); }
        ...
        Self::verify_owned_account(invoke_context, 2, treasury)?;
        Self::collect_from_signer(invoke_context, sender_key, treasury, record.amount)
    }

    fn process_create_subscription(invoke_context: &mut InvokeContext, record: crate::state::SubscriptionRecord) -> Result<(), InstructionError> {
        let subscriber_key = Self::signer_key(invoke_context, 1)?;
        if subscriber_key != record.subscriber { return Err(InstructionError::IncorrectAuthority); }
        let treasury = {
            let transaction_context = &invoke_context.transaction_context;
            let instruction_context = transaction_context.get_current_instruction_context()?;
            instruction_context.check_number_of_instruction_accounts(3)?;
            ...
            let mut state = SocialMonetizationStateAccount::deserialize_padded(state_account.get_data()).map_err(|_| InstructionError::InvalidAccountData)?;
            state.ensure_initialized().map_err(Self::map_program_error)?;
            if !state.config.subscriptions_enabled { return Err(Self::map_program_error(SocialMonetizationError::SubscriptionsDisabled.into())); }
            if record.amount_per_period == 0 || record.period_seconds == 0 { return Err(Self::map_program_error(SocialMonetizationError::InvalidAmount.into())); }
            if record.valid_until_unix <= record.started_at_unix { return Err(Self::map_program_error(SocialMonetizationError::InvalidSubscriptionWindow.into())); }
            if state.subscription_exists(&record.subscription_id) { return Err(Self::map_program_error(SocialMonetizationError::DuplicateSubscription.into())); }
        ...
        Self::verify_owned_account(invoke_context, 2, treasury)?;
        Self::collect_from_signer(invoke_context, subscriber_key, treasury, record.amount_per_period)
    }

    fn process_renew_subscription(invoke_context: &mut InvokeContext, subscription_id: [u8; 32], valid_until_unix: i64) -> Result<(), InstructionError> {
        let subscriber_key = Self::signer_key(invoke_context, 1)?;
        let (treasury, amount) = {
            let transaction_context = &invoke_context.transaction_context;
            let instruction_context = transaction_context.get_current_instruction_context()?;
            ...
            let mut state = SocialMonetizationStateAccount::deserialize_padded(state_account.get_data()).map_err(|_| InstructionError::InvalidAccountData)?;
            state.ensure_initialized().map_err(Self::map_program_error)?;
            let subscription = state.subscriptions.iter_mut().find(|entry| entry.subscription_id == subscription_id && entry.subscriber == subscriber_key)
                .ok_or_else(|| Self::map_program_error(SocialMonetizationError::SubscriptionNotFound.into()))?;
            if subscription.state != SubscriptionState::Active { return Err(Self::map_program_error(SocialMonetizationError::SubscriptionNotActive.into())); }
            if valid_until_unix <= subscription.valid_until_unix { return Err(Self::map_program_error(SocialMonetizationError::InvalidSubscriptionWindow.into())); }
        ...
        Self::verify_owned_account(invoke_context, 2, treasury)?;
        Self::collect_from_signer(invoke_context, subscriber_key, treasury, amount)
    }

    fn process_cancel_subscription(invoke_context: &mut InvokeContext, subscription_id: [u8; 32]) -> Result<(), InstructionError> {
        let subscriber_key = Self::signer_key(invoke_context, 1)?;
        let transaction_context = &invoke_context.transaction_context;
        let instruction_context = transaction_context.get_current_instruction_context()?;
    ...
    fn process_unlock_paid_content(invoke_context: &mut InvokeContext, record: crate::state::PaidContentUnlockRecord) -> Result<(), InstructionError> {
        let buyer_key = Self::signer_key(invoke_context, 1)?;
        if buyer_key != record.buyer { return Err(InstructionError::IncorrectAuthority); }
        let treasury = {
            let transaction_context = &invoke_context.transaction_context;
            let instruction_context = transaction_context.get_current_instruction_context()?;
            instruction_context.check_number_of_instruction_accounts(3)?;
            ...
            let mut state = SocialMonetizationStateAccount::deserialize_padded(state_account.get_data()).map_err(|_| InstructionError::InvalidAccountData)?;
            state.ensure_initialized().map_err(Self::map_program_error)?;
            if !state.config.paid_content_enabled { return Err(Self::map_program_error(SocialMonetizationError::PaidContentDisabled.into())); }
            if record.amount == 0 { return Err(Self::map_program_error(SocialMonetizationError::InvalidAmount.into())); }
            if state.unlock_exists(&record.unlock_id) { return Err(Self::map_program_error(SocialMonetizationError::DuplicateUnlock.into())); }
        ...
        Self::verify_owned_account(invoke_context, 2, treasury)?;
        Self::collect_from_signer(invoke_context, buyer_key, treasury, record.amount)
    }
    ...
    fn process_claim(invoke_context: &mut InvokeContext, creator: Pubkey, amount: u64) -> Result<(), InstructionError> {
        ...
        if revenue.claimable_amount < amount { return Err(Self::map_program_error(SocialMonetizationError::NothingToClaim.into())); }
    ...
    fn process_read(invoke_context: &mut InvokeContext, creator: Option<Pubkey>) -> Result<(), InstructionError> {
        let return_data = {
            ...
            if *state_account.get_owner() != crate::id() { return Err(InstructionError::InvalidAccountOwner); }
            let state = SocialMonetizationStateAccount::deserialize_padded(state_account.get_data()).map_err(|_| InstructionError::InvalidAccountData)?;
            state.ensure_initialized().map_err(Self::map_program_error)?;
            if let Some(creator) = creator {
                to_vec(&state.revenues.iter().find(|entry| entry.creator == creator).cloned()).map_err(|_| InstructionError::InvalidAccountData)?
            } else {
    ...
    fn credit_creator(state: &mut SocialMonetizationStateAccount, creator: Pubkey, amount: u64) {
        ...
        if let Some(existing) = state.revenues.iter_mut().find(|entry| entry.creator == creator) {
            ...
            state.revenues.push(CreatorRevenueAccount { creator, total_earned: creator_amount as u128, total_claimed: 0, claimable_amount: creator_amount });
        }
    }

    fn creator_net_amount(state: &SocialMonetizationStateAccount, amount: u64) -> u64 {
        let fee = (amount as u128).saturating_mul(state.config.platform_fee_bps as u128) / 10_000u128;
        amount.saturating_sub(fee as u64)
    }

    fn write_back(state_account: &mut aeko_sdk::transaction_context::BorrowedAccount<'_>, state: &SocialMonetizationStateAccount) -> Result<(), InstructionError> {
...
mod tests {
    use {
        crate::state::{CreatorRevenueAccount, MonetizationConfig, PaidContentUnlockRecord, SocialMonetizationStateAccount, SubscriptionRecord, SubscriptionState},
        aeko_sdk::pubkey::Pubkey,
    };

    fn test_state(platform_fee_bps: u16) -> SocialMonetizationStateAccount {
        SocialMonetizationStateAccount::new(MonetizationConfig { authority: Pubkey::new_unique(), treasury: Pubkey::new_unique(), platform_fee_bps, subscriptions_enabled: true, paid_content_enabled: true })
    }
    ...
    fn subscription_lifecycle_changes_state() {
        ...
        state.subscriptions.push(SubscriptionRecord { subscription_id: [1u8; 32], creator, subscriber, amount_per_period: 100, period_seconds: 30, started_at_unix: 10, valid_until_unix: 40, state: SubscriptionState::Active });
    ...
    fn duplicate_unlock_detection_is_possible() {
        ...
        let mut state = test_state(0);
        let unlock = PaidContentUnlockRecord { unlock_id: [9u8; 32], content_id: [7u8; 32], creator, buyer, amount: 200, unlocked_at_unix: 33 };
