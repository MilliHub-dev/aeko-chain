...
mod tests {
    ...
    fn test_process_nonce_ix_ok() {
        ...
        let new_recent_blockhashes_account =
            aeko_sdk::recent_blockhashes_account::create_account_with_data_for_test(
    ...
    fn test_nonce_initialize_with_empty_recent_blockhashes_fail() {
        ...
        #[allow(deprecated)]
        let new_recent_blockhashes_account =
            aeko_sdk::recent_blockhashes_account::create_account_with_data_for_test(vec![]);
        process_instruction(
            &serialize(&SystemInstruction::InitializeNonceAccount(nonce_address)).unwrap(),
            vec![
                (nonce_address, nonce_account),
    ...
    fn test_assign_native_loader_and_transfer() {
        for size in [0, 10] {
            ...
            let accounts = process_instruction(
                &bincode::serialize(&SystemInstruction::Assign {
                    owner: aeko_sdk::native_loader::id(),
                })
                .unwrap(),
                vec![(pubkey, account.clone())],
                vec![AccountMeta {
            ...
            assert_eq!(accounts[0].owner(), &aeko_sdk::native_loader::id());
            assert_eq!(accounts[0].lamports(), 100);
            ...
            assert_eq!(accounts[1].owner(), &aeko_sdk::native_loader::id());
