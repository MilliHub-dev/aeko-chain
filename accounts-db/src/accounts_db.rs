...
use {
    ...
    smallvec::SmallVec,
    aeko_measure::{measure::Measure, measure_us},
    aeko_nohash_hasher::{IntMap, IntSet},
...
impl aeko_frozen_abi::abi_example::AbiExample for AccountsDb {
    fn example() -> Self {
...
impl AccountsDb {
    ...
    pub fn create_account(
        ...
        for t in 0..num_vote {
            let pubkey = aeko_sdk::pubkey::new_rand();
            let account =
                AccountSharedData::new((num + t + 1) as u64, space, &aeko_vote_program::id());
            pubkeys.push(pubkey);
            let ancestors = vec![(slot, 0)].into_iter().collect();
            assert!(self.load_without_fixed_root(&ancestors, &pubkey).is_none());
            self.store_for_tests(slot, &[(&pubkey, &account)]);
...
pub mod tests {
    ...
    fn test_get_keys_to_unref_ancient() {
        ...
        let pubkey4 = aeko_sdk::pubkey::new_rand();
    ...
    fn test_verify_bank_capitalization() {
        for pass in 0..2 {
            ...
            let native_account_pubkey = aeko_sdk::pubkey::new_rand();
            db.store_for_tests(
                some_slot,
                &[(
                    &native_account_pubkey,
                    &aeko_sdk::native_loader::create_loadable_account_for_test("foo"),
                )],
            );
            db.add_root_and_flush_write_cache(some_slot);
            db.update_accounts_hash_for_tests(some_slot, &ancestors, true, true);
    ...
    fn test_storage_finder() {
        ...
        let lamports = 100;
        let data_len = 8190;
        let account = AccountSharedData::new(lamports, data_len, &aeko_sdk::pubkey::new_rand());
    ...
    fn test_shrink_candidate_slots() {
        ...
        let pubkeys: Vec<_> = (0..pubkey_count)
            .map(|_| aeko_sdk::pubkey::new_rand())
            .collect();
    ...
    fn test_account_balance_for_capitalization_sysvar() {
        let normal_sysvar = aeko_sdk::account::create_account_for_test(
            &aeko_sdk::slot_history::SlotHistory::default(),
        );
    ...
    fn test_flush_accounts_cache() {
        ...
        let root5 = 5;
        let root6 = 6;
        let unrooted_key = aeko_sdk::pubkey::new_rand();
        let key5 = aeko_sdk::pubkey::new_rand();
        let key6 = aeko_sdk::pubkey::new_rand();
    ...
    fn test_calculate_storage_count_and_alive_bytes() {
        ...
        let shared_key = aeko_sdk::pubkey::new_rand();
        let account = AccountSharedData::new(1, 1, AccountSharedData::default().owner());
        let slot0 = 0;
    ...
    fn test_calculate_storage_count_and_alive_bytes_2_accounts() {
        let accounts = AccountsDb::new_single_for_tests();
        let keys = [
            aeko_sdk::pubkey::Pubkey::from([0; 32]),
            aeko_sdk::pubkey::Pubkey::from([255; 32]),
        ];
        accounts.accounts_index.set_startup(Startup::Startup);
    ...
    fn test_clean_accounts_with_last_full_snapshot_slot() {
        ...
        let accounts_db = AccountsDb::new_single_for_tests();
        let pubkey = aeko_sdk::pubkey::new_rand();
        let owner = aeko_sdk::pubkey::new_rand();
        let space = 0;
