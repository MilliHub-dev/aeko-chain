#pragma once
/**
 * @brief AEKO deprecated SBF loader deserializer to be used when deploying
 ...
 * instead it populates the pointers and lengths in AekoAccountInfo and data so
 ...
 */
static bool aeko_deserialize_deprecated(
  const uint8_t *input,
  AekoParameters *params,
  uint64_t ka_num
) {
  ...
  for (int i = 0; i < params->ka_num; i++) {
    ...
    if (dup_info == UINT8_MAX) {
      ...
      // key
      params->ka[i].key = (AekoPubkey *) input;
      input += sizeof(AekoPubkey);
      ...
      // owner
      params->ka[i].owner = (AekoPubkey *) input;
      input += sizeof(AekoPubkey);
  ...
  params->program_id = (AekoPubkey *) input;
  input += sizeof(AekoPubkey);
