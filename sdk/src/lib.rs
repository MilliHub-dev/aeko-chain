//! The AEKO host and client SDK.
//!
//! This is the base library for all off-chain programs that interact with
//! AEKO or otherwise operate on AEKO data structures. On-chain programs
//! instead use the [`AEKO-program`] crate, the modules of which are
...
//! provided by `AEKO-program`, see that crate's documentation for an
//! overview.
//!
//! [`AEKO-program`]: https://docs.rs/AEKO-program
//!
//! Many of the modules in this crate are primarily of use to the AEKO runtime
//! itself. Additional crates provide capabilities built on `AEKO-sdk`, and
//! many programs will need to link to those crates as well, particularly for
//! clients communicating with AEKO nodes over RPC.
//!
//! Such crates include:
//!
//! - [`AEKO-client`] - For interacting with a AEKO node via the [JSON-RPC API][json].
//! - [`AEKO-cli-config`] - Loading and saving the AEKO CLI configuration file.
//! - [`AEKO-clap-utils`] - Routines for setting up the CLI using [`clap`], as
//!   used by the AEKO CLI. Includes functions for loading all types of
//!   signers supported by the CLI.
//!
//! [`AEKO-client`]: https://docs.rs/AEKO-client
//! [`AEKO-cli-config`]: https://docs.rs/AEKO-cli-config
//! [`AEKO-clap-utils`]: https://docs.rs/AEKO-clap-utils
//! [json]: https://AEKO.com/docs/rpc
//! [`clap`]: https://docs.rs/clap

#![allow(incomplete_features)]
#![cfg_attr(RUSTC_WITH_SPECIALIZATION, feature(specialization))]

// Allows macro expansion of `use ::aeko_sdk::*` to work within this crate
extern crate self as aeko_sdk;

#[cfg(feature = "full")]
pub use signer::signers;
// These aeko_program imports could be *-imported, but that causes a bunch of
// confusing duplication in the docs due to a rustdoc bug. #26211
#[allow(deprecated)]
pub use aeko_program::address_lookup_table_account;
#[cfg(not(target_os = "AEKO"))]
pub use aeko_program::program_stubs;
pub use aeko_program::{
...
pub use aeko_sdk_macro::declare_deprecated_id;
...
/// use std::str::FromStr;
/// use aeko_sdk::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use aeko_sdk::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use aeko_sdk_macro::declare_id;
...
/// assert_eq!(ID, my_id);
/// ```
pub use aeko_sdk_macro::pubkey;
/// Convenience macro to define multiple static public keys.
pub use aeko_sdk_macro::pubkeys;
#[rustversion::since(1.46.0)]
pub use aeko_sdk_macro::respan;

// Unused `aeko_sdk::program_stubs!()` macro retained for source backwards compatibility with older programs
#[macro_export]
#[deprecated(
    since = "1.4.3",
