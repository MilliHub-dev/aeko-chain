...
//!
//! [np]: https://docs.aeko.network/runtime/programs#secp256k1-program
//!
...
//! [`ecrecover`] precompile. For that AEKO provides the [`secp256k1_recover`]
//! syscall.
//!
//! [secp256k1]: https://en.bitcoin.it/wiki/Secp256k1
//! [`secp256k1_program`]: aeko_program::secp256k1_program
//! [`secp256k1_recover`]: aeko_program::secp256k1_recover
...
//!
//! The `aeko_sdk` crate provides few APIs for building the instructions and
//! transactions necessary for properly using the secp256k1 native program.
//! Many steps must be done manually.
//!
//! The `aeko_program` crate provides no APIs to assist in interpreting
...
//! and callers should ensure they use the same version of `bincode` as the AEKO SDK.
//! This data structure is not provided to AEKO programs,
...
//!
//! **The AEKO `secp256k1_recover` function does not prevent signature
//! malleability**. This is in contrast to the Bitcoin secp256k1 library, which
//! does prevent malleability by default. AEKO accepts signatures with `S`
...
//! to parse the secp256k1 instruction data from within a AEKO program.
//!
//! ```no_run
//! mod secp256k1_defs {
//!     use aeko_program::program_error::ProgramError;
...
//!
//! This example has two components: a AEKO program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the AEKO program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The AEKO program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//! containing many signatures, and a AEKO program that parses them all. This
...
//! transaction. In the _previous_ example, the AEKO program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the AEKO program supports signatures, messages, and
...
pub fn new_secp256k1_instruction(
    ...
    Instruction {
        program_id: aeko_sdk::secp256k1_program::id(),
        accounts: vec![],
...
///
/// `feature_set` is the set of active AEKO features. It is used to enable or
