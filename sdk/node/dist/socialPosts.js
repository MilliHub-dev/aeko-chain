...
export function socialPostsProgramId() {
    return encodeBase58(SOCIAL_POSTS_PROGRAM_ID_BYTES);
}
export function buildCanonicalPostPayload(input) {
...
export function serializeCanonicalPostPayload(input) {
...
export function buildPreparedAnchorPostTransaction(input) {
    const instructionData = concatBytes(Uint8Array.from([1]), encodeFixed32(input.postId), encodePubkey(input.creator), encodeFixed32(input.contentHash), encodeFixed32(input.metadataHash), encodeString(input.contentUri), encodeOptionFixed32(input.parentPostId), encodePostKind(input.postKind), encodeI64(input.createdAtUnix), Uint8Array.from([0]), encodeVisibilityClass(input.visibility), Uint8Array.from([0]), encodeOptionFixed32(input.signatureRef));
...
function normalizeSignatureBytes(signature, encoding) {
...
function encodeOptionFixed32(value) {
    if (!value) {
...
function encodeVisibilityClass(value) {
    switch (value) {
        case 'public':
            return Uint8Array.from([0]);
        case 'followersOnly':
