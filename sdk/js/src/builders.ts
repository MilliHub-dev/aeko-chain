...
export interface InitializeWalletPermissionsTransactionInput
  extends WalletPermissionsTransactionInput {
...
function encodeBincodeString(value: string): Uint8Array {
  const encoded = new TextEncoder().encode(value);
  return concatBytes(encodeU64(encoded.length), encoded);
}

function encodeOptionString(value?: string | null): Uint8Array {
  if (!value) return Uint8Array.from([0]);
...
function encodeVec<T>(items: T[], encodeItem: (item: T) => Uint8Array): Uint8Array {
  return concatBytes(encodeU32(items.length), ...items.map(encodeItem));
}

function encodeOptionU16(value?: number | null): Uint8Array {
  if (value === null || typeof value === 'undefined') return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodeU16(value));
}

function encodeOptionU64(value?: number | null): Uint8Array {
  if (value === null || typeof value === 'undefined') return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodeU64(value));
}

function encodeNestedOptionString(value?: string | null): Uint8Array {
  if (typeof value === 'undefined') return Uint8Array.from([0]);
  if (value === null) return Uint8Array.from([1, 0]);
  return concatBytes(Uint8Array.from([1, 1]), encodeString(value));
}

function encodeNestedOptionU64(value?: number | null): Uint8Array {
  if (typeof value === 'undefined') return Uint8Array.from([0]);
  if (value === null) return Uint8Array.from([1, 0]);
  return concatBytes(Uint8Array.from([1, 1]), encodeU64(value));
}

function encodeOptionBool(value?: boolean): Uint8Array {
  if (typeof value === 'undefined') return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodeBool(value));
}

function encodeOptionRole(value?: PermissionRole): Uint8Array {
  if (typeof value === 'undefined') return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodePermissionRole(value));
}

function encodeOptionSpendLimit(value?: SpendLimitPolicy): Uint8Array {
  if (typeof value === 'undefined') return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodeSpendLimitPolicy(value));
}

function encodeOptionPubkey(value?: PublicKeyString): Uint8Array {
  if (!value) return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodePubkey(value));
}

function encodeOptionPubkeyVec(value?: PublicKeyString[]): Uint8Array {
  if (typeof value === 'undefined') return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodeVec(value, encodePubkey));
}

function encodeOptionStringVec(value?: string[]): Uint8Array {
  if (typeof value === 'undefined') return Uint8Array.from([0]);
  return concatBytes(Uint8Array.from([1]), encodeVec(value, encodeHashString));
}

function encodeHashString(value: string): Uint8Array {
...
function encodeProgramPolicyMode(value: ProgramPolicyMode): Uint8Array {
  return Uint8Array.from([value === 'allow_by_default' ? 1 : 0]);
}

function encodePermissionRole(value: PermissionRole): Uint8Array {
  switch (value) {
    case 'owner':
...
function encodePermissionStatus(value: DelegatePermission['status']): Uint8Array {
  switch (value) {
    case 'active':
      return Uint8Array.from([0]);
    case 'revoked':
...
function encodeTokenSpendCap(value: TokenSpendCap): Uint8Array {
  return concatBytes(
    encodePubkey(value.mint),
    encodeOptionU64(value.maxSingleTx),
    encodeOptionU64(value.maxDaily),
  );
}

function encodeSpendLimitPolicy(value: SpendLimitPolicy): Uint8Array {
  return concatBytes(
    encodeOptionU64(value.maxSingleTxAeko),
    encodeOptionU64(value.maxDailyAeko),
    encodeVec(value.tokenCaps, encodeTokenSpendCap),
  );
}

function encodeDelegatePermission(value: DelegatePermission): Uint8Array {
  return concatBytes(
    encodePubkey(value.delegate),
    encodePermissionRole(value.role),
    encodeOptionString(value.label ?? null),
    encodePermissionStatus(value.status),
    encodeU64(value.validFromEpoch),
    encodeOptionU64(value.validUntilEpoch),
    encodeSpendLimitPolicy(value.spendLimit),
    encodeVec(value.programAllowlist, encodePubkey),
    encodeVec(value.tokenAllowlist, encodePubkey),
    encodeVec(value.appScopeHashes, encodeHashString),
    encodeBool(value.requiresReauth),
    encodeOptionU64(undefined),
    encodeOptionU64(undefined),
  );
}

function encodeMetadata(metadata: Token721MetadataInput): Uint8Array {
  ...
  return concatBytes(
    ...
    ...attributes.map((attribute) =>
      concatBytes(encodeString(attribute.traitType), encodeString(attribute.value)),
    ),
  );
}

function encodeInstructionData(
  action: Token721ActionInput['action'],
  args: {
    name?: string;
    symbol?: string;
    baseUri?: string | null;
  ...
  switch (action) {
    ...
    case 'mint':
      return concatBytes(
        Uint8Array.from([1]),
        encodeU64(args.tokenId ?? 0),
        decodeBase58(args.owner ?? ''),
        decodeBase58(args.creator ?? ''),
        encodeU16(args.royaltyBps ?? 0),
        encodeMetadata(args.metadata ?? { name: '', uri: '' }),
      );
    case 'freeze':
      return Uint8Array.from([2]);
    case 'thaw':
...
function compileInstruction(input: Token721ActionInput) {
...
function buildCreateAccountWithSeedInstruction(input: {
  payer: PublicKeyString;
  createdAccount: PublicKeyString;
  base: PublicKeyString;
  seed: string;
  lamports: number;
  space: number;
  ownerProgram: PublicKeyString;
}) {
  return {
    ...
    data: concatBytes(
      encodeU32(3),
      decodeBase58(input.base),
      encodeBincodeString(input.seed),
      encodeU64(input.lamports),
      encodeU64(input.space),
      decodeBase58(input.ownerProgram),
    ),
  };
}
...
export function token721ProgramId(): PublicKeyString {
...
export function buildPreparedToken721Transaction(input: Token721ActionInput): string {
...
export function estimateTokenAccountSpace(input: { metadata: Token721MetadataInput }): number {
...
export function buildPreparedCollectionSetupTransaction(input: CollectionSetupInput): string {
  return buildPreparedTransaction({
    payer: input.payer,
    recentBlockhash: input.recentBlockhash,
    instructions: [
...
export function buildPreparedMintWithAccountSetupTransaction(
  input: MintWithAccountSetupInput,
): string {
  return buildPreparedTransaction({
    payer: input.payer,
    recentBlockhash: input.recentBlockhash,
...
function buildWalletPermissionsInstruction(
  accounts: WalletPermissionAccountsInput,
  data: Uint8Array,
) {
  return {
...
export function buildPreparedInitializeWalletPermissionsTransaction(
  input: InitializeWalletPermissionsTransactionInput,
): string {
  const instruction = buildWalletPermissionsInstruction(
    input.accounts,
    concatBytes(
      Uint8Array.from([0]),
      encodePubkey(input.wallet),
      encodeString(input.did),
      encodeU64(input.currentEpoch),
      encodeProgramPolicyMode(input.defaultProgramPolicy),
    ),
  );
...
export function buildPreparedGrantDelegateTransaction(
  input: GrantDelegateTransactionInput,
): string {
  const instruction = buildWalletPermissionsInstruction(
    input.accounts,
    concatBytes(
      Uint8Array.from([1]),
      encodeDelegatePermission(input.delegatePermission),
      encodeU64(input.currentEpoch),
...
export function buildPreparedUpdateDelegateTransaction(
  input: UpdateDelegateTransactionInput,
): string {
  const instruction = buildWalletPermissionsInstruction(
    input.accounts,
    concatBytes(
      Uint8Array.from([2]),
      encodePubkey(input.delegate),
      encodeOptionRole(input.role),
      encodeNestedOptionString(input.label),
      encodeNestedOptionU64(input.validUntilEpoch),
      encodeOptionSpendLimit(input.spendLimit),
      encodeOptionPubkeyVec(input.programAllowlist),
      encodeOptionPubkeyVec(input.tokenAllowlist),
      encodeOptionStringVec(input.appScopeHashes),
      encodeOptionBool(input.requiresReauth),
      encodeU64(input.currentEpoch),
      encodeU64(input.currentSlot),
    ),
  );
...
export function buildPreparedRevokeDelegateTransaction(
  input: RevokeDelegateTransactionInput,
): string {
  const instruction = buildWalletPermissionsInstruction(
    input.accounts,
    concatBytes(
...
export function buildPreparedFreezeWalletTransaction(
  input: FreezeWalletTransactionInput,
): string {
  const instruction = buildWalletPermissionsInstruction(
    input.accounts,
    concatBytes(
      Uint8Array.from([4]),
      encodeOptionU16(input.reasonCode),
      encodeOptionU64(input.reauthRequiredUntilEpoch),
      encodeU64(input.currentEpoch),
      encodeU64(input.currentSlot),
    ),
  );
...
export function buildPreparedUnfreezeWalletTransaction(
  input: UnfreezeWalletTransactionInput,
): string {
  const instruction = buildWalletPermissionsInstruction(
    input.accounts,
    concatBytes(
...
export function buildPreparedRecordDelegateUsageTransaction(
  input: RecordDelegateUsageTransactionInput,
): string {
  const instruction = buildWalletPermissionsInstruction(
    input.accounts,
    concatBytes(
      Uint8Array.from([6]),
      encodePubkey(input.delegate),
      encodeOptionPubkey(input.targetProgram),
      encodeOptionPubkey(input.mint),
      encodeU64(input.amount),
      encodeU64(input.dayIndex),
      encodeU64(input.currentEpoch),
      encodeU64(input.currentSlot),
    ),
  );
