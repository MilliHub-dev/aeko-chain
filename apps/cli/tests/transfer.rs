...
use {
    ...
    aeko_cli_output::{parse_sign_only_reply_string, OutputFormat},
    aeko_faucet::faucet::run_local_faucet,
    aeko_rpc_client::rpc_client::RpcClient,
    aeko_rpc_client_nonce_utils::blockhash_query::{self, BlockhashQuery},
    aeko_sdk::{
...
fn test_transfer() {
    ...
    request_and_confirm_airdrop(&rpc_client, &config, &sender_pubkey, aeko_to_lamports(5.0))
        .unwrap();
    check_balance!(aeko_to_lamports(5.0), &rpc_client, &sender_pubkey);
    ...
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(aeko_to_lamports(1.0)),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
    ...
    check_balance!(
        aeko_to_lamports(4.0) - fee_one_sig,
        &rpc_client,
        &sender_pubkey
    );
    check_balance!(aeko_to_lamports(1.0), &rpc_client, &recipient_pubkey);

    // Plain ole transfer, failure due to InsufficientFundsForSpendAndFee
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(aeko_to_lamports(4.0)),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
        dump_transaction_message: false,
    ...
    request_and_confirm_airdrop(&rpc_client, &offline, &offline_pubkey, aeko_to_lamports(1.0))
        .unwrap();
    check_balance!(aeko_to_lamports(1.0), &rpc_client, &offline_pubkey);

    // Offline transfer
    let blockhash = rpc_client.get_latest_blockhash().unwrap();
    offline.command = CliCommand::Transfer {
        amount: SpendAmount::Some(aeko_to_lamports(0.5)),
        to: recipient_pubkey,
        from: 0,
        sign_only: true,
        dump_transaction_message: false,
    ...
    process_command(&config).unwrap();
    check_balance!(
        aeko_to_lamports(0.5) - fee_one_sig,
        &rpc_client,
        &offline_pubkey
    );
    check_balance!(aeko_to_lamports(1.5), &rpc_client, &recipient_pubkey);
    ...
    process_command(&config).unwrap();
    check_balance!(
        aeko_to_lamports(4.0) - fee_one_sig - fee_two_sig - minimum_nonce_balance,
        &rpc_client,
        &sender_pubkey,
    );

    // Fetch nonce hash
    let nonce_hash = aeko_rpc_client_nonce_utils::get_account_with_commitment(
        &rpc_client,
        &nonce_account.pubkey(),
        CommitmentConfig::processed(),
    )
    .and_then(|ref a| aeko_rpc_client_nonce_utils::data_from_account(a))
    .unwrap()
    .blockhash();
    ...
    process_command(&config).unwrap();
    check_balance!(
        aeko_to_lamports(3.0) - 2 * fee_one_sig - fee_two_sig - minimum_nonce_balance,
        &rpc_client,
        &sender_pubkey,
    );
    check_balance!(aeko_to_lamports(2.5), &rpc_client, &recipient_pubkey);
    let new_nonce_hash = aeko_rpc_client_nonce_utils::get_account_with_commitment(
        &rpc_client,
        &nonce_account.pubkey(),
        CommitmentConfig::processed(),
    )
    ...
    process_command(&config).unwrap();
    check_balance!(
        aeko_to_lamports(3.0) - 3 * fee_one_sig - fee_two_sig - minimum_nonce_balance,
        &rpc_client,
        &sender_pubkey,
    );
    ...
    offline.signers = vec![&default_offline_signer];
    offline.command = CliCommand::Transfer {
        amount: SpendAmount::Some(aeko_to_lamports(0.4)),
        to: recipient_pubkey,
        from: 0,
        sign_only: true,
        dump_transaction_message: false,
    ...
    process_command(&config).unwrap();
    check_balance!(
        aeko_to_lamports(0.1) - 2 * fee_one_sig,
        &rpc_client,
        &offline_pubkey
    );
    check_balance!(aeko_to_lamports(2.9), &rpc_client, &recipient_pubkey);
}
...
fn test_transfer_multisession_signing() {
    ...
    request_and_confirm_airdrop(
        ...
        aeko_to_lamports(1.0) + 2 * fee_two_sig,
    )
    .unwrap();
    check_balance!(
        aeko_to_lamports(43.0),
    ...
    fee_payer_config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(aeko_to_lamports(42.0)),
        to: to_pubkey,
        from: 1,
        sign_only: true,
        dump_transaction_message: false,
    ...
    check_balance!(
        aeko_to_lamports(1.0),
        &rpc_client,
        &offline_from_signer.pubkey(),
    );
    check_balance!(
        aeko_to_lamports(1.0) + fee_two_sig,
        &rpc_client,
        &offline_fee_payer_signer.pubkey(),
    );
    check_balance!(aeko_to_lamports(42.0), &rpc_client, &to_pubkey);
}
...
fn test_transfer_with_seed() {
    ...
    request_and_confirm_airdrop(&rpc_client, &config, &sender_pubkey, aeko_to_lamports(1.0))
        .unwrap();
    request_and_confirm_airdrop(&rpc_client, &config, &derived_address, aeko_to_lamports(5.0))
        .unwrap();
    check_balance!(aeko_to_lamports(1.0), &rpc_client, &sender_pubkey);
    check_balance!(aeko_to_lamports(5.0), &rpc_client, &derived_address);
    check_balance!(0, &rpc_client, &recipient_pubkey);
    ...
    // Transfer with seed
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(aeko_to_lamports(5.0)),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
        dump_transaction_message: false,
    ...
    process_command(&config).unwrap();
    check_balance!(aeko_to_lamports(1.0) - fee, &rpc_client, &sender_pubkey);
    check_balance!(aeko_to_lamports(5.0), &rpc_client, &recipient_pubkey);
