use {
    ...
    aeko_rpc_client_api::config::RpcGetVoteAccountsConfig,
    ...
    aeko_sdk::{
        ...
        native_token::lamports_to_aeko, pubkey::Pubkey, system_instruction::SystemError,
            ...
            SubCommand::with_name("withdraw-from-vote-account")
                ...
                .arg(pubkey!(
                    Arg::with_name("destination_account_pubkey")
                        ...
                        .required(true),
                    "The recipient of withdrawn AEKO."
                ))
                .arg(
                    Arg::with_name("amount")
                        ...
                        .help(
                            "The amount to withdraw, in AEKO; accepts keyword ALL, which for this \
            ...
            SubCommand::with_name("close-vote-account")
                ...
                .arg(pubkey!(
                    Arg::with_name("destination_account_pubkey")
                        ...
                        .value_name("RECIPIENT_ADDRESS")
                        .required(true),
                    "The recipient of all withdrawn AEKO."
                ))
                .arg(
                    Arg::with_name("authorized_withdrawer")
                        .long("authorized-withdrawer")
...
pub fn process_create_vote_account(
    ...
    let vote_account_address = if let Some(seed) = seed {
        Pubkey::create_with_seed(&vote_account_pubkey, seed, &aeko_vote_program::id())?
    } else {
        vote_account_pubkey
    };
    ...
    if !sign_only {
            ...
            rpc_client.get_account_with_commitment(&vote_account_address, config.commitment)
        {
            if let Some(vote_account) = response.value {
                let err_msg = if vote_account.owner == aeko_vote_program::id() {
...
pub(crate) fn get_vote_account(
    ...
    if vote_account.owner != aeko_vote_program::id() {
    ...
    if !sign_only {
        ...
        if let SpendAmount::Some(withdraw_amount) = withdraw_amount {
            ...
            if balance_remaining < minimum_balance && balance_remaining != 0 {
                return Err(CliError::BadParameter(format!(
                    "Withdraw amount too large. The vote account balance must be at least {} AEKO \
                     to remain rent exempt",
                    lamports_to_aeko(minimum_balance)
                ))
                .into());
            }
        }
...
mod tests {
    ...
    fn test_parse_command() {
        ...
        let authed = aeko_sdk::pubkey::new_rand();
        let (keypair_file, mut tmp_file) = make_tmp_file();
        let keypair = Keypair::new();
        write_keypair(&keypair, tmp_file.as_file_mut()).unwrap();
