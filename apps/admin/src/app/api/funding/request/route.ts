...
import { FundingError, requestFundingApproval } from '@/lib/funding-store'
...
export async function POST(req: NextRequest) {
  ...
  try {
    const request = await requestFundingApproval(address, 'public')
    return respond({ data: request }, { status: 202 })
  } catch (err) {
    if (err instanceof FundingError) {
      const headers: Record<string, string> = {}
      if (typeof err.extra.retryAfterSeconds === 'number') {
