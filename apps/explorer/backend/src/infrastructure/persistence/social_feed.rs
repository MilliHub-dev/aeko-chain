    ...
    pub async fn list_social_feed_page(&self, creator: Option<&str>, cursor: Option<&SocialFeedCursor>, limit: usize) -> Result<Vec<SocialPostRecord>> {
        let rows = sqlx::query(r#"
            ...
            WHERE ($1::TEXT IS NULL OR creator = $1)
              AND ($2::BIGINT IS NULL OR created_at_unix < $2 OR (created_at_unix = $2 AND post_id > $3))
            ...
            .fetch_all(&self.pool).await.context("listing cursor-paginated Social feed")?;
    ...
    pub async fn get_social_thread(&self, post_id: &str, limit: usize) -> Result<Vec<SocialPostRecord>> {
        let rows = sqlx::query(r#"
            SELECT post_id, creator, content_hash, metadata_hash, content_uri, parent_post_id,
                   post_kind, created_at_unix, edited_at_unix, visibility, moderation_state, signature_ref
            FROM posts
            WHERE post_id = $1 OR parent_post_id = $1
            ORDER BY CASE WHEN post_id = $1 THEN 0 ELSE 1 END ASC,
                     created_at_unix ASC, post_id ASC
            LIMIT $2
            "#)
            .bind(post_id)
            .bind(i64::try_from(limit).context("social thread limit exceeds BIGINT")?)
            .fetch_all(&self.pool).await.context("loading Social thread")?;
        rows.into_iter().map(post_from_row).collect()
    }
}
