...
impl PostgresRepository {
    ...
    pub async fn ping(&self) -> Result<()> {
        ...
        if value != 1 {
            return Err(anyhow!(
                "Explorer PostgreSQL readiness query returned {value}"
            ));
        }
        Ok(())
    }
    ...
    pub async fn latest_persisted_block_identity(&self) -> Result<Option<(u64, String)>> {
        let row: Option<(i64, String)> =
            sqlx::query_as("SELECT slot, blockhash FROM blocks ORDER BY slot DESC LIMIT 1")
                .fetch_optional(&self.pool)
                .await
    ...
    pub async fn next_core_slot(&self, configured_start_slot: u64) -> Result<u64> {
        let row: Option<i64> =
            sqlx::query_scalar("SELECT next_slot FROM indexer_cursors WHERE stream = 'core'")
                .fetch_optional(&self.pool)
                .await
    ...
    pub async fn latest_projection_slot(&self, stream: &str) -> Result<Option<u64>> {
        let next: Option<i64> =
            sqlx::query_scalar("SELECT next_slot FROM indexer_cursors WHERE stream = $1")
                .bind(stream)
                .fetch_optional(&self.pool)
                .await
        ...
        match next {
            ...
            Some(value) => Err(anyhow!(
                "persisted {stream} projection cursor is negative: {value}"
            )),
        }
    }
