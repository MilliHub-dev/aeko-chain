...
fn reputation_score(profile: &aeko_social_anti_spam_program::state::AntiSpamProfile, current_epoch: u64) -> u16 {
    let gated = profile.gated_until_epoch.is_some_and(|until| until >= current_epoch);
    let penalty = profile.spam_flags.saturating_mul(500)
        .saturating_add(profile.slash_count.saturating_mul(1000))
        .saturating_add(if gated { 2000 } else { 0 });
    10_000u16.saturating_sub(penalty.min(10_000))
}

fn post_kind_label(value: PostKind) -> &'static str {
    match value { PostKind::Original => "original", PostKind::Reply => "reply", PostKind::Repost => "repost", PostKind::Quote => "quote" }
}
fn visibility_label(value: VisibilityClass) -> &'static str {
    match value { VisibilityClass::Public => "public", VisibilityClass::FollowersOnly => "followers-only", VisibilityClass::Permissioned => "permissioned", VisibilityClass::Paid => "paid" }
}
fn moderation_label(value: ModerationState) -> &'static str {
    match value { ModerationState::Active => "active", ModerationState::ReducedReach => "reduced-reach", ModerationState::HiddenByApp => "hidden-by-app", ModerationState::LockedByProtocol => "locked-by-protocol" }
}
fn engagement_label(value: EngagementActionKind) -> &'static str {
    match value { EngagementActionKind::Like => "like", EngagementActionKind::Comment => "comment", EngagementActionKind::Repost => "repost", EngagementActionKind::Quote => "quote", EngagementActionKind::Share => "share", EngagementActionKind::Save => "save" }
}
fn stake_state_label(value: SocialStakeState) -> &'static str {
    match value { SocialStakeState::Active => "active", SocialStakeState::CoolingDown => "cooling-down", SocialStakeState::Closed => "closed", SocialStakeState::Slashed => "slashed" }
}
fn subscription_state_label(value: SubscriptionState) -> &'static str {
    match value { SubscriptionState::Active => "active", SubscriptionState::Expired => "expired", SubscriptionState::Canceled => "canceled" }
}
