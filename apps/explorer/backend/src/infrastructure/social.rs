...
impl CanonicalChainDataSource {
    ...
    fn required_state<T: borsh::BorshDeserialize>(
        ...
        let address = address.ok_or_else(|| {
            anyhow!("{label} state is missing from the canonical bootstrap registry")
        })?;
        let state = self
            .rpc
            .fetch_owned_state::<T>(address, program_id, label)?;
...
impl ChainDataSource for CanonicalChainDataSource {
    ...
    fn fetch_social_snapshot(&self, slot: u64) -> Result<SocialSnapshot> {
        ...
        let (anti_spam_address, anti_spam_state) = self
            .required_state::<SocialAntiSpamStateAccount>(
                registry.anti_spam.as_deref(),
                aeko_social_anti_spam_program::id(),
                "social-anti-spam",
            )?;
        let (monetization_address, monetization_state) = self
            .required_state::<SocialMonetizationStateAccount>(
        ...
        {
            return Err(anyhow!(
                "one or more canonical Social state accounts are not initialized"
            ));
        }
        ...
        let reward_accounts = rewards_state
            .creators
            .iter()
            .map(|entry| SocialRewardAccountRecord {
            ...
            .collect::<Vec<_>>();
        let reward_epochs = rewards_state
            .epochs
            .iter()
            .map(|entry| CreatorRewardRecord {
                creator: entry.creator.to_string(),
                epoch: entry.epoch,
            ...
            .collect::<Vec<_>>();
        let reward_settlements = rewards_state
            .settlements
            .iter()
            .map(|entry| RewardSettlementRecord {
                epoch: entry.epoch,
        ...
        let stakes = staking_state
            ...
            .collect::<Vec<_>>();
        let stake_yields = staking_state
            .yield_records
            .iter()
            .map(|entry| StakeYieldRecord {
                epoch: entry.epoch,
        ...
        let anti_spam_profiles = anti_spam_state
            .profiles
            .iter()
            .map(|profile| AntiSpamProfileRecord {
        ...
        let tips = monetization_state
            .tips
            .iter()
            .map(|entry| CreatorTipRecord {
            ...
            .collect::<Vec<_>>();
        let subscriptions = monetization_state
            .subscriptions
            .iter()
            ...
            .collect::<Vec<_>>();
        let unlocks = monetization_state
            .unlocks
            .iter()
            .map(|entry| PaidContentUnlockRecord {
            ...
            .collect::<Vec<_>>();
        let revenues = monetization_state
            .revenues
            .iter()
            .map(|entry| CreatorRevenueRecord {
                creator: entry.creator.to_string(),
                total_earned: entry.total_earned.to_string(),
                total_claimed: entry.total_claimed.to_string(),
                claimable_amount: entry.claimable_amount,
        ...
        let domains = vec![
            SocialDomainSnapshotRecord {
                domain: "posts".to_string(),
                state_account: posts_address,
                program_id: aeko_social_posts_program::id().to_string(),
                slot,
                epoch,
                item_count: posts.len() + engagement.len(),
            },
            SocialDomainSnapshotRecord {
                domain: "rewards".to_string(),
                state_account: rewards_address,
                program_id: aeko_social_rewards_program::id().to_string(),
                slot,
                epoch,
                item_count: reward_accounts.len() + reward_epochs.len() + reward_settlements.len(),
            },
            SocialDomainSnapshotRecord {
                domain: "staking".to_string(),
                state_account: staking_address,
                program_id: aeko_social_staking_program::id().to_string(),
                slot,
                epoch,
                item_count: stakes.len() + stake_yields.len(),
            },
            SocialDomainSnapshotRecord {
                domain: "anti-spam".to_string(),
                state_account: anti_spam_address,
                program_id: aeko_social_anti_spam_program::id().to_string(),
                slot,
                epoch,
                item_count: anti_spam_profiles.len(),
            },
            SocialDomainSnapshotRecord {
                domain: "monetization".to_string(),
                state_account: monetization_address,
                program_id: aeko_social_monetization_program::id().to_string(),
                slot,
                epoch,
                item_count: tips.len() + subscriptions.len() + unlocks.len() + revenues.len(),
            },
        ];
...
fn post_kind_label(value: PostKind) -> &'static str {
    match value {
...
fn visibility_label(value: VisibilityClass) -> &'static str {
    match value {
...
fn moderation_label(value: ModerationState) -> &'static str {
    match value {
...
fn engagement_label(value: EngagementActionKind) -> &'static str {
    match value {
...
fn stake_state_label(value: SocialStakeState) -> &'static str {
    match value {
...
fn subscription_state_label(value: SubscriptionState) -> &'static str {
    match value {
