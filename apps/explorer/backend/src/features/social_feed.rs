...
async fn list_feed(
    ...
    let next_cursor = if has_more {
        items
            .last()
            .map(|post| format_cursor(post.created_at_unix, &post.post_id))
    } else {
        None
    };
    Ok(response::data(
...
async fn get_thread(
    ...
    let items = state
        .repository
        .get_social_thread(&post_id, clamp_limit(params.limit).min(250))
        .await?;
...
fn parse_cursor(value: &str) -> ApiResult<SocialFeedCursor> {
    let (timestamp, post_id) = value
        .split_once(':')
        .ok_or_else(|| ApiError::BadRequest("invalid social feed cursor".to_string()))?;
    let created_at_unix = timestamp
        .parse::<i64>()
        .map_err(|_| ApiError::BadRequest("invalid social feed cursor timestamp".to_string()))?;
    if post_id.is_empty() {
        return Err(ApiError::BadRequest(
            "invalid social feed cursor post id".to_string(),
        ));
    }
    Ok(SocialFeedCursor {
        created_at_unix,
