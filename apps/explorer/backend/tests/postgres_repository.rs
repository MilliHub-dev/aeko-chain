use {
    aeko_explorer_backend::{
        ...
        models::{
            ...
            SearchResultRecord, SocialDomainSnapshotRecord,
...
async fn postgres_cursor_filters_assets_and_social_are_durable() -> Result<()> {
    ...
    assert_eq!(
        repository.latest_projection_slot("assets").await?,
        Some(slot)
    );
        ...
        .persist_social_snapshot(SocialSnapshot {
            ...
            domains: vec![
                SocialDomainSnapshotRecord {
                    domain: "posts".into(),
                    state_account: "posts-state".into(),
                    program_id: "posts-program".into(),
                    slot: social_slot,
                    epoch: 77,
                    item_count: 2,
                },
                SocialDomainSnapshotRecord {
                    domain: "rewards".into(),
                    state_account: "rewards-state".into(),
                    program_id: "rewards-program".into(),
                    slot: social_slot,
                    epoch: 77,
                    item_count: 3,
                },
                SocialDomainSnapshotRecord {
                    domain: "staking".into(),
                    state_account: "staking-state".into(),
                    program_id: "staking-program".into(),
                    slot: social_slot,
                    epoch: 77,
                    item_count: 2,
                },
                SocialDomainSnapshotRecord {
                    domain: "anti-spam".into(),
                    state_account: "anti-spam-state".into(),
                    program_id: "anti-spam-program".into(),
                    slot: social_slot,
                    epoch: 77,
                    item_count: 1,
                },
                SocialDomainSnapshotRecord {
                    domain: "monetization".into(),
                    state_account: "monetization-state".into(),
                    program_id: "monetization-program".into(),
                    slot: social_slot,
                    epoch: 77,
                    item_count: 4,
                },
            ...
            posts: vec![SocialPostRecord {
                ...
                metadata_hash: "metadata-hash".into(),
                content_uri: "https://example.invalid/post".into(),
                parent_post_id: None,
                post_kind: "original".into(),
                created_at_unix: 1_700_000_100,
                edited_at_unix: Some(1_700_000_101),
                visibility: "public".into(),
                moderation_state: "active".into(),
                signature_ref: Some("signature-ref".into()),
            }],
            engagement: vec![EngagementRecord {
                proof_id: "proof-1".into(),
                actor: signer.clone(),
                target_creator: signer.clone(),
                target_post_id: Some("post-1".into()),
                action_kind: "like".into(),
                action_weight: 1,
                slot: social_slot,
                unix_timestamp: 1_700_000_102,
                replay_guard: "guard-1".into(),
            }],
            reward_accounts: vec![SocialRewardAccountRecord {
                creator: signer.clone(),
                total_earned: "100".into(),
                total_claimed: "40".into(),
                claimable_amount: 60,
                last_settled_epoch: 77,
            }],
            reward_epochs: vec![CreatorRewardRecord {
                creator: signer.clone(),
                epoch: 77,
                earned_points: "123".into(),
                reward_amount: 100,
                claimed_amount: 40,
                claimable_amount: 60,
                penalty_bps: 50,
            }],
            reward_settlements: vec![RewardSettlementRecord {
                epoch: 77,
                reward_pool_amount: 1000,
                total_effective_points: "123".into(),
                settled_creator_count: 1,
            }],
            stakes: vec![SocialStakeRecord {
                position_id: "stake-1".into(),
                staker: signer.clone(),
                creator: signer.clone(),
                staked_amount: 500,
                activated_at_epoch: 70,
                unlock_epoch: Some(80),
                state: "active".into(),
                accumulated_yield: 12,
                claimed_yield: 2,
            }],
            stake_yields: vec![StakeYieldRecord {
                epoch: 77,
                position_id: "stake-1".into(),
                creator: signer.clone(),
                staker: signer.clone(),
            ...
            anti_spam_profiles: vec![AntiSpamProfileRecord {
                wallet: signer.clone(),
                post_count_window: 3,
                engagement_count_window: 8,
                spam_flags: 1,
                gated_until_epoch: None,
                slash_count: 0,
                last_flagged_at_unix: Some(1_700_000_103),
                reputation_score: 950,
            }],
            tips: vec![CreatorTipRecord {
                tip_id: "tip-1".into(),
                creator: signer.clone(),
            ...
            unlocks: vec![PaidContentUnlockRecord {
                unlock_id: "unlock-1".into(),
                content_id: "content-1".into(),
                creator: signer.clone(),
                buyer: "buyer-1".into(),
                amount: 5,
                unlocked_at_unix: 1_700_000_105,
            }],
            revenues: vec![CreatorRevenueRecord {
                creator: signer.clone(),
                total_earned: "34".into(),
                total_claimed: "10".into(),
                claimable_amount: 24,
            }],
        })
        .await?;
    ...
    assert_eq!(
        repository.latest_projection_slot("social").await?,
        Some(social_slot)
    );
    ...
    assert_eq!(
        repository
            .list_reward_accounts(Some(&signer), 5)
            .await?
            .len(),
        1
    );
    ...
    assert_eq!(
        repository
            .list_anti_spam_profiles(Some(&signer), 5)
            .await?
            .len(),
        1
    );
    ...
    assert_eq!(
        repository
            .list_creator_revenues(Some(&signer), 5)
            .await?
            .len(),
        1
    );
