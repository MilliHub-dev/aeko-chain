...
export function fetchPostsPage(baseUrl, {
  creator = '',
  before = null,
  after = null,
  postKind = '',
  ...
  return fetchConsoleApi(baseUrl, `/posts${queryString({
    creator: creator || undefined,
    before,
    after,
    postKind: postKind || undefined,
    visibility: visibility || undefined,
    limit: safeLimit,
  })}`);
}

export function fetchPost(baseUrl, postId) {
  return fetchConsoleApi(baseUrl, `/posts/${encodeURIComponent(postId)}`);
}

export function fetchPostEngagement(baseUrl, postId, { actionKind = '', limit = 100 } = {}) {
  const safeLimit = Math.max(1, Math.min(Number(limit) || 100, 100));
  return fetchConsoleApi(baseUrl, `/engagement${queryString({
    postId,
...
export function fetchCreatorSocial(baseUrl, creator, wallet = '') {
  ...
  const requests = {
    posts: `/posts?creator=${encodedCreator}&limit=100`,
    stakes: `/stakes?creator=${encodedCreator}&limit=100`,
    rewards: `/rewards?creator=${encodedCreator}&limit=100`,
    rewardAccounts: `/social/reward-accounts?creator=${encodedCreator}&limit=100`,
    stakeYields: `/social/stake-yields?creator=${encodedCreator}&limit=100`,
    antiSpam: `/social/anti-spam?wallet=${encodedCreator}&limit=20`,
    tips: `/social/tips?creator=${encodedCreator}&limit=100`,
    subscriptions: `/social/subscriptions?creator=${encodedCreator}&limit=100`,
    unlocks: `/social/unlocks?creator=${encodedCreator}&limit=100`,
    revenues: `/social/revenues?creator=${encodedCreator}&limit=100`,
    walletStakes: `/stakes?wallet=${encodedWallet}&limit=100`,
    walletSubscriptions: `/social/subscriptions?subscriber=${encodedWallet}&limit=100`,
    walletUnlocks: `/social/unlocks?buyer=${encodedWallet}&limit=100`,
  };
