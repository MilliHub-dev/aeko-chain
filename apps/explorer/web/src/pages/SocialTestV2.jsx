...
export default function SocialTestV2() {
  ...
  async function runFullFlow() {
    if (busy) return;
    setBusy(true);
    updateStep('full', 'running', 'Running signed Social custody flow end to end.');
    try {
      const target = wallet || ensureWallet();
      const readiness = await refreshStatus();
      await ensureFunded(target);
      ...
      await refreshStatus();
      updateStep('full', 'pass', 'Post + engagement + real stake escrow + real tip transfer + Explorer projection all passed.');
    } catch (error) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-16 pt-24 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div><div className="mb-2 text-sm font-medium text-aeko-accent">Explorer acceptance lab</div><h1 className="mb-4 text-4xl font-bold md:text-5xl">AEKO SocialFi Custody E2E</h1><p className="max-w-3xl text-lg text-gray-400">Exercise signed Social transactions through public RPC, prove actual wallet/vault balance movement, then require Explorer PostgreSQL projections to observe the same records before a step can pass.</p></div>
        <button type="button" disabled={busy} onClick={runFullFlow} className="inline-flex min-h-[46px] items-center justify-center gap-2 rounded-xl bg-aeko-accent px-5 text-sm font-semibold text-black disabled:opacity-50"><Sparkles size={17} /> Run custody flow</button>
      </div>

      <div className="mb-8 flex gap-3 rounded-2xl border border-amber-400/30 bg-amber-400/10 p-5"><AlertTriangle size={20} className="mt-0.5 shrink-0 text-amber-300" /><div className="text-sm text-amber-100/90"><div className="mb-1 font-semibold">Testnet keys move real testnet lamports</div><p>Browser-local wallets remain unencrypted test keys. The stake and tip steps now fail unless the selected wallet loses the expected principal/value and the configured program-owned vault gains exactly that amount. Do not use these keys for assets with real economic value.</p></div></div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4"><ActionCard icon={Send} title="1. Post" description="Sign an on-chain post and require /posts/:id to index it." state={steps.post} disabled={busy} onClick={() => runAction('post', runPost)} /><ActionCard icon={Heart} title="2. Like" description="Sign engagement and require its proof with a canonical chain slot." state={steps.like} disabled={busy || !lastPost} onClick={() => runAction('like', (target, resolved) => runLike(target, resolved, lastPost))} /><ActionCard icon={ShieldCheck} title="3. Stake custody" description="Move 0.01 AEKO from the wallet into the program-owned principal vault and index the exact position." state={steps.stake} disabled={busy} onClick={() => runAction('stake', runStake)} /><ActionCard icon={CircleDollarSign} title="4. Tip custody" description="Move 0.01 AEKO from the sender into the program-owned monetization treasury and index the exact tip." state={steps.tip} disabled={busy} onClick={() => runAction('tip', runTip)} /></div>

      <section className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03]"><div className="flex items-center justify-between gap-4 border-b border-white/10 px-5 py-4"><div><h2 className="font-semibold">Acceptance evidence</h2><p className="mt-1 text-xs text-gray-500">PASS requires observable RPC balance movement and/or the corresponding Explorer projection.</p></div><button type="button" disabled={busy} onClick={() => runAction('rewards', runRewardsRead)} className="rounded-lg border border-white/15 px-3 py-2 text-xs hover:bg-white/5 disabled:opacity-50">Verify rewards read</button></div><div className="divide-y divide-white/10">{[['full','Full custody flow'],['fund','Wallet funding'],['post','Post → Explorer'],['like','Like → Explorer'],['stake','Wallet → stake vault → Explorer'],['tip','Wallet → treasury → Explorer'],['rewards','Rewards read'],['readiness','Five-domain readiness']].map(([key,label]) => <div key={key} className="grid grid-cols-1 items-start gap-2 px-5 py-4 md:grid-cols-[220px_100px_1fr] md:gap-4"><div className="text-sm text-gray-200">{label}</div><StepState state={steps[key]} /><div><div className="text-sm text-gray-400">{steps[key]?.message || 'Not run yet.'}</div>{steps[key]?.detail ? <div className="mt-1 break-all font-mono text-[11px] text-gray-600">{steps[key].detail}</div> : null}</div></div>)}</div></section>
    </div>
  );
}
