...
function SocialWorkspace({ rpcUrl, explorerApiUrl, explorerUrl, wallets, balances, socialPulse, socialStateAccount, socialAntiSpamStateAccount }) {
  ...
  const publish = async () => {
    if (!wallet || !body.trim() || !socialStateAccount || !socialAntiSpamStateAccount) return;
    setBusy('post');
    setResult(null);
    try {
      const recentBlockhash = await getLatestBlockhash(rpcUrl);
