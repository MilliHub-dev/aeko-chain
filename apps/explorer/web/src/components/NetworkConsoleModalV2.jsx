...
function EndpointStatus({ icon: Icon, label, status, detail }) {
...
function AccountsWorkspace({ rpcUrl, explorerApiUrl, explorerUrl, wallets, setWallets, balances, refreshBalance }) {
  ...
  const wallet = wallets.find((item) => item.id === selectedId) || null;
    ...
    <div className="grid gap-5 xl:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="space-y-4">
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="mt-1 text-xs leading-relaxed text-gray-500">Keys are generated and signed in this browser. Balance and transaction state come from the validator RPC; profile/history enrichment comes from Explorer API.</div>
        ...
        <div className="space-y-2">
          {wallets.map((item) => (
            <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`w-full rounded-xl border p-3 text-left ${selectedId === item.id ? 'border-aeko-accent/35 bg-aeko-accent/[0.07]' : 'border-white/10 bg-white/[0.025] hover:bg-white/[0.05]'}`}>
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-3">
        <EndpointStatus icon={Server} label="JSON-RPC" status={rpcState.status} detail={rpcUrl} />
        <EndpointStatus icon={Server} label="Explorer API" status={overview ? 'ready' : 'error'} detail={explorerApiUrl} />
        <EndpointStatus icon={Wifi} label="WebSocket" status={wsState} detail={websocketUrl} />
      </div>
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
...
function SocialWorkspace({ rpcUrl, explorerApiUrl, explorerUrl, wallets, balances, socialPulse, socialStateAccount }) {
  ...
  useEffect(() => {
    if (!wallets.some((item) => item.id === walletId)) setWalletId(wallets[0]?.id || '');
  }, [wallets, walletId]);
  ...
  const refresh = useCallback(async () => {
    try {
      const [discovered, projected] = await Promise.all([
        ...
        fetchSocialProjection(explorerApiUrl, { wallet: wallet?.address || '', limit: 50 }),
      ]);
  ...
  }, [rpcUrl, explorerApiUrl, wallet?.address]);
  ...
  const publish = async () => {
    ...
    try {
      ...
      await confirmSignature(rpcUrl, signature);
      setBody('');
      setResult({ kind: 'success', message: 'Social post confirmed on chain.', signature });
      await refresh();
    } catch (publishError) {
  ...
  const posts = useMemo(() => [...(chainState?.posts || [])].sort((a, b) => b.createdAtUnix - a.createdAtUnix), [chainState]);
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
      <div className="space-y-4">
        ...
        <div className="divide-y divide-white/10 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.02]">
          {posts.map((post) => <article key={post.postId} className="p-4"><div className="flex gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/5"><UserRound size={15} /></div><div className="min-w-0 flex-1"><div className="text-[11px] text-gray-500"><span className="font-medium text-white">{shortAddress(post.creator)}</span> · {new Date(post.createdAtUnix * 1000).toLocaleString()}</div><div className="mt-2 whitespace-pre-wrap break-words text-sm text-gray-100">{post.contentUri}</div><button type="button" onClick={() => like(post)} disabled={!wallet || Boolean(busy)} className="mt-3 inline-flex h-8 items-center gap-1.5 rounded-full px-2 text-xs text-gray-500 hover:bg-pink-500/10 hover:text-pink-300 disabled:opacity-40">{busy === `like:${post.postId}` ? <Loader2 size={12} className="animate-spin" /> : <Heart size={12} />} {engagement.likeCountByPost.get(post.postId) || 0}</button></div></div></article>)}
          {posts.length === 0 ? <div className="p-10 text-center text-sm text-gray-500">No on-chain posts yet.</div> : null}
        </div>
      </div>
      <aside className="space-y-3">
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4"><div className="text-sm font-semibold text-white">Explorer projection</div><div className="mt-1 text-xs leading-relaxed text-gray-500">These are durable PostgreSQL views indexed from the five canonical Social state accounts. They are not generated in the browser.</div></div>
...
export default function NetworkConsoleModalV2({ open, onClose, tab, onTabChange, rpcUrl, websocketUrl, network, explorerApiUrl, explorerUrl }) {
  ...
  useEffect(() => {
    ...
    Promise.all(wallets.map((wallet) => refreshBalance(wallet.address)));
    const timer = window.setInterval(refreshInfrastructure, 30000);
  ...
  useEffect(() => {
    ...
    wallets.forEach((wallet) => {
      stops.push(client.subscribeAccount(wallet.address, () => refreshBalance(wallet.address)));
    });
    client.connect();
    return () => {
