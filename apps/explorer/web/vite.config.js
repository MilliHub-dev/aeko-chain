...
export default defineConfig(({ command, mode }) => {
  ...
  const publicRpc = clean(env.AEKO_TESTNET_RPC_URL)
  const publicWs = clean(env.AEKO_TESTNET_WS_URL)
  const testnetUpstream = clean(env.AEKO_TESTNET_EXPLORER_API_URL)

  const mainnetRpc = clean(env.AEKO_MAINNET_RPC_URL)
  const mainnetWs = clean(env.AEKO_MAINNET_WS_URL)
  const mainnetUpstream = clean(env.AEKO_MAINNET_EXPLORER_API_URL)
  ...
  const localnetUpstreamEnv = clean(env.AEKO_LOCALNET_EXPLORER_API_URL)
