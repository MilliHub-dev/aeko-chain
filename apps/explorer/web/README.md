...
`src/utils/explorerApi.js` is the production Explorer read boundary. Production endpoints come from `window.__AEKO_RUNTIME_CONFIG__`, generated by `docker/explorer-ui-entrypoint.sh` from the `AEKO_PUBLIC_*` deployment variables. `VITE_AEKO_*` values are reserved for local development and explicit preview builds.
...
- local Vite development uses the loopback `VITE_AEKO_LOCAL_*` trio;
- explicit Vite preview builds may use `VITE_AEKO_TESTNET_*` / `VITE_AEKO_MAINNET_*`.
