...
use {
    ...
    aeko_sdk::clock::DEFAULT_MS_PER_SLOT,
};
use {
    crate::{
    ...
    aeko_client::connection_cache::Protocol,
    ...
    aeko_streamer::sendmmsg::{batch_send, SendPktsError},
    std::{
            ...
            .filter_map(|(pubkey, stake)| {
                ...
                if let Some(Ok(peer_repair_addr)) = peer_repair_addr {
                    ...
                    Some((
                        *pubkey,
                        peer_repair_addr,
                        (*stake / aeko_sdk::native_token::LAMPORTS_PER_AEKO) as u32,
                    ))
                } else {
                    None
                }
...
mod test {
    ...
    pub fn test_request_repair_for_shred_from_address() {
        ...
        let mut packets = vec![aeko_sdk::packet::Packet::default(); 1];
        let _recv_count = aeko_streamer::recvmmsg::recv_mmsg(&reader, &mut packets[..]).unwrap();
