...
pub use aeko_perf::report_target_features;
use {
    crate::{
    ...
    aeko_entry::poh::compute_hash_time_ns,
    aeko_geyser_plugin_manager::{
    ...
    aeko_streamer::{socket::SocketAddrSpace, streamer::StakedNodes},
    aeko_turbine::{self, broadcast_stage::BroadcastStageType},
    aeko_unified_scheduler_pool::DefaultSchedulerPool,
    aeko_vote_program::vote_state,
    aeko_wen_restart::wen_restart::wait_for_wen_restart,
    std::{
...
pub struct Validator {
    ...
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<aeko_net_utils::IpEchoServer>,
    ...
    turbine_quic_endpoint_join_handle: Option<aeko_turbine::quic_endpoint::AsyncTryJoinHandle>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if aeko_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(aeko_net_utils::ip_echo_server(
                tcp_listener,
        ...
        let (
            ...
            aeko_turbine::quic_endpoint::new_quic_endpoint(
        ...
        datapoint_info!(
            ...
            ("version", aeko_version::version!(), String),
    ...
    pub fn join(self) {
        ...
        if let Some(turbine_quic_endpoint) = &self.turbine_quic_endpoint {
            aeko_turbine::quic_endpoint::close_quic_endpoint(turbine_quic_endpoint);
        }
...
fn maybe_warp_slot(
    ...
    if let Some(warp_slot) = config.warp_slot {
        ...
        bank_forks.insert(Bank::warp_from_parent(
            ...
            &Pubkey::default(),
            warp_slot,
            aeko_accounts_db::accounts_db::CalcAccountsHashDataSource::Storages,
        ));
        bank_forks.set_root(
            warp_slot,
            accounts_background_request_sender,
...
mod tests {
    use {
        ...
        aeko_entry::entry,
        aeko_gossip::contact_info::{ContactInfo, LegacyContactInfo},
        aeko_ledger::{
            blockstore, create_new_tmp_ledger, genesis_utils::create_genesis_config_with_leader,
            get_tmp_ledger_path_auto_delete,
        },
        aeko_sdk::{genesis_config::create_genesis_config, poh_config::PohConfig},
        aeko_tpu_client::tpu_client::{
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            target_tick_duration: Duration::from_millis(aeko_sdk::clock::MS_PER_TICK),
            // make PoH rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * aeko_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
