...
impl From<aeko_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: aeko_rpc_client_nonce_utils::Error) -> Self {
        match error {
            aeko_rpc_client_nonce_utils::Error::Client(client_error) => {
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            get_clap_app(
                crate_name!(),
                crate_description!(),
                aeko_version::version!(),
            )
            .gen_completions_to("aeko", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
...
mod tests {
    use {
        ...
        aeko_rpc_client_nonce_utils::blockhash_query,
        aeko_sdk::{
            pubkey::Pubkey,
            signature::{
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(aeko_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_process_command() {
        ...
        let new_authorized_pubkey = aeko_sdk::pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = aeko_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = aeko_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = aeko_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = aeko_sdk::pubkey::new_rand();
