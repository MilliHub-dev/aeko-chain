use {
    ...
    aeko_account_decoder::validator_info::{
    ...
    aeko_cli_output::{CliValidatorInfo, CliValidatorInfoVec},
    aeko_config_program::{config_instruction, get_config_data, ConfigKeys, ConfigState},
...
fn verify_keybase(
    ...
    if let Some(keybase_username) = keybase_username.as_str() {
        let url =
            format!("https://keybase.pub/{keybase_username}/aeko/validator-{validator_pubkey:?}");
...
fn parse_validator_info(
    ...
    if account.owner != aeko_config_program::id() {
    ...
    fn validator_info_subcommands(self) -> Self {
        self.subcommand(
            SubCommand::with_name("validator-info")
                .about("Publish/get Validator info on Aeko")
                .setting(AppSettings::SubcommandRequiredElseHelp)
                .subcommand(
                    SubCommand::with_name("publish")
                        .about("Publish Validator info on Aeko")
                        .arg(
                ...
                .subcommand(
                    SubCommand::with_name("get")
                        .about("Get and parse Aeko Validator info")
                        .arg(
                            Arg::with_name("info_pubkey")
                                .index(1)
                                .value_name("PUBKEY")
...
pub fn process_set_validator_info(
    ...
    let all_config = rpc_client.get_program_accounts(&aeko_config_program::id())?;
...
mod tests {
    ...
    fn test_parse_validator_info_not_validator_info_account() {
        assert!(parse_validator_info(
            &Pubkey::default(),
            &Account {
                owner: aeko_sdk::pubkey::new_rand(),
                ..Account::default()
            }
        )
        .unwrap_err()
