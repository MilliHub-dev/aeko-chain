...
fn check_env_path_for_bin_dir(config: &Config) {
    ...
    if !found {
        println!(
            "\nPlease update your PATH environment variable to include the aeko programs:\n    PATH=\"{}:$PATH\"\n",
...
pub fn init(
    ...
    let config = {
        // Write new config file only if different, so that running |aeko-install init|
...
fn github_release_download_url(release_semver: &str) -> String {
    format!(
        "https://github.com/aeko-chain/aeko/releases/download/v{}/aeko-release-{}.tar.bz2",
        release_semver,
...
fn release_channel_download_url(release_channel: &str) -> String {
    format!(
        "https://release.aeko.chain/{}/aeko-release-{}.tar.bz2",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn release_channel_version_url(release_channel: &str) -> String {
    format!(
        "https://release.aeko.chain/{}/aeko-release-{}.yml",
        release_channel,
        crate::build_env::TARGET
    )
}
...
pub fn info(config_file: &str, local_info_only: bool, eval: bool) -> Result<(), String> {
    ...
    if eval {
        ...
        config
            ...
            .and_then(|channel| {
                println!("AEKO_INSTALL_ACTIVE_CHANNEL={channel}",);
                Option::<String>::None
            });
        return Ok(());
    }
...
fn check_for_newer_github_release(
    ...
    while page == 1 || releases.len() == PER_PAGE {
        let url = reqwest::Url::parse_with_params(
            "https://api.github.com/repos/aeko-chain/aeko/releases",
            &[
