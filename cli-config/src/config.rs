...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "aeko", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The AEKO CLI configuration.
...
pub struct Config {
    ...
    ///
    /// [rpcdocs]: https://docs.aeko.chain/core/clusters
    ...
    /// documentation for `aeko_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `aeko_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from AEKO addresses to human-readable names.
    ///
    ...
    /// `aeko_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
