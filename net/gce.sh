...
if [[ $cloudProvider = ec2 ]]; then
  ...
  sshPrivateKey="$HOME/.ssh/aeko-net-id_$prefix"
else
    ...
    if [[ ! -r $sshPrivateKey ]]; then
      ...
      (
        ...
        for i in $(seq 1 60); do
          set -x
          cloud_FetchFile "$nodeName" "$nodeIp" /aeko-scratch/id_ecdsa "$sshPrivateKey" "$nodeZone" &&
            cloud_FetchFile "$nodeName" "$nodeIp" /aeko-scratch/id_ecdsa.pub "$sshPrivateKey.pub" "$nodeZone" &&
              break
          set +x
  ...
  recordInstanceIp() {
    ...
    (
      ...
      for i in $(seq 1 60); do
        (
          set -x
          timeout --preserve-status --foreground 20s ssh "${sshOptions[@]}" "$publicIp" "ls -l /aeko-scratch/.instance-startup-complete"
        )
        ret=$?
        if [[ $ret -eq 0 ]]; then
...
if [[ -f /aeko-scratch/.instance-startup-complete ]]; then
  echo reboot
  $(
    cd "$here"/scripts/
    if "$enableGpu"; then
  ...
  if [[ -x ~aeko/aeko/on-reboot ]]; then
    sudo -u aeko ~aeko/aeko/on-reboot
  fi
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  To block until setup is complete, run:
    $ until [[ -f /aeko-scratch/.instance-startup-complete ]]; do sleep 1; done

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$(creationInfo)
EOM

# Place the generated private key at /aeko-scratch/id_ecdsa so it's retrievable by anybody
# who is able to log into this machine
mkdir -m 0777 /aeko-scratch
cat > /aeko-scratch/id_ecdsa <<EOK
$(cat "$sshPrivateKey")
EOK
cat > /aeko-scratch/id_ecdsa.pub <<EOK
$(cat "$sshPrivateKey.pub")
EOK
chmod 444 /aeko-scratch/id_ecdsa
...
$(
  ...
  cat \
    ...
    create-aeko-user.sh \
    aeko-user-authorized_keys.sh \
    add-testnet-aeko-user-authorized_keys.sh \
    install-ag.sh \
    install-certbot.sh \
    install-earlyoom.sh \
    install-iftop.sh \
...
cat >/aeko-scratch/gce-self-destruct.sh <<'EOS'
...
cat >/aeko-scratch/gce-self-destruct-ps1.sh <<'EOS'
#!/usr/bin/env bash
source "$(dirname "$0")/gce-self-destruct.sh"
gce_self_destruct_ps1
EOS
chmod +x /aeko-scratch/gce-self-destruct-ps1.sh

# Append MOTD and PS1 replacement to .profile
cat >>~aeko/.profile <<'EOS'

# Print self-destruct countdown on login
source "/aeko-scratch/gce-self-destruct.sh"
gce_self_destruct_motd

# Add self-destruct countdown to terminal prompt
export PS1='\[\e]0;\u@\h: \w\a\]${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]$(/aeko-scratch/gce-self-destruct-ps1.sh):\[\033[01;34m\]\w\[\033[00m\]\$ '
EOS
EOSD
    cat <<EOSD
...
tmpfsMountPoint=/mnt/aeko-accounts
...
touch /aeko-scratch/.instance-startup-complete
