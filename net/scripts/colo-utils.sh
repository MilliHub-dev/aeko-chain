...
declare -r AEKO_LOCK_FILE="/home/aeko/.aeko.lock"
...
colo_instance_run() {
  ...
  declare OUT
  set +e
  OUT=$(ssh -l aeko -o "StrictHostKeyChecking=no" -o "ConnectTimeout=3" -n "${IP}" "${CMD}" 2>&1)
  declare RC=$?
  set -e
  while read -r LINE; do
...
colo_whoami() {
  ...
  done < <(colo_instance_run_foreach "[ -n \"\${AEKO_USER}\" ] && echo -e \"\${AEKO_USER}\\x1fEOL\"")
  echo "${ME}"
}

COLO_AEKO_USER=""
colo_get_aeko_user() {
  if [ -z "${COLO_AEKO_USER}" ]; then
    COLO_AEKO_USER=$(colo_whoami)
  fi
  echo "${COLO_AEKO_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${AEKO_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${AEKO_LOCK_FILE}" && flock -s 9 && . "${AEKO_LOCK_FILE}" && exec 9>&-
  echo -e "\${AEKO_LOCK_USER}\\x1f\${AEKO_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
...
SSH_AUTHORIZED_KEYS='$("${__colo_here}"/add-datacenter-aeko-user-authorized_keys.sh 2> /dev/null)'
