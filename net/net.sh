...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip aeko-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'aeko-install deploy ...' for the
...
startCommon() {
  ...
  [[ -z "$externalNodeSshKey" ]] || ssh-copy-id -f -i "$externalNodeSshKey" "${sshOptions[@]}" "aeko@$ipAddress"
...
startBootstrapLeader() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./aeko/net/remote/remote-node.sh \
         $deployMethod \
         bootstrap-validator \
         $entrypointIp \
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./aeko/net/remote/remote-client.sh $deployMethod $entrypointIp \
...
startClients() {
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "aeko-bench-tps" "$i"
    else
...
sanity() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$bootstrapLeader" \
      "./aeko/net/remote/remote-sanity.sh $bootstrapLeader $sanityExtraArgs \"$RUST_LOG\""
  ) || ok=false
  $ok || exit 1
  ...
  if [[ -z $skipBlockstreamerSanity && -n $blockstreamer ]]; then
    ...
    (
      ...
      ssh "${sshOptions[@]}" "$blockstreamer" \
        "./aeko/net/remote/remote-sanity.sh $blockstreamer $sanityExtraArgs \"$RUST_LOG\""
    ) || ok=false
    $ok || exit 1
  fi
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying aeko-install update: $updatePlatform"
    (
      set -x

      scripts/aeko-install-update-manifest-keypair.sh "$updatePlatform"

      timeout 30s scp "${sshOptions[@]}" \
        update_manifest_keypair.json "$bootstrapLeader:aeko/update_manifest_keypair.json"

      # shellcheck disable=SC2029 # remote-deploy-update.sh args are expanded on client side intentionally
      ssh "${sshOptions[@]}" "$bootstrapLeader" \
        "./aeko/net/remote/remote-deploy-update.sh $releaseChannel $updatePlatform"
    ) || ok=false
    $ok || exit 1
  done
}
...
deploy() {
  ...
  if ! $waitForNodeInit; then
    ...
    for ipAddress in "${validatorIpList[@]}" "${blockstreamerIpList[@]}"; do
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "./aeko/net/remote/remote-node-wait-init.sh $((600 - timeWaited))"
    done
  fi
...
stopNode() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "PS4=\"$PS4\" ./aeko/net/remote/cleanup.sh"
  ) >> "$logFile" 2>&1 &
...
logs)
  ...
  fetchRemoteLog() {
    ...
    (
      set -x
      timeout 30s scp "${sshOptions[@]}" \
        "$ipAddress":aeko/"$log".log "$netLogDir"/remote-"$log"-"$ipAddress".log
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    for i in "${!validatorIpList[@]}"; do
      "$here"/ssh.sh aeko@"${validatorIpList[$i]}" 'aeko/scripts/net-shaper.sh' \
      "$netemCommand" ~aeko/aeko/"$remoteNetemConfigFile" "${#validatorIpList[@]}" "$i"
    done
  else
    ...
    for ipAddress in "${validatorIpList[@]}"; do
      "$here"/ssh.sh aeko@"$ipAddress" 'aeko/scripts/netem.sh delete < aeko/netem.cfg || true'
    done

    # Start netem on required nodes
    for ((i=0; i<num_nodes; i++ )); do :
      "$here"/ssh.sh aeko@"${validatorIpList[$i]}" "echo $netemConfig > aeko/netem.cfg; aeko/scripts/netem.sh add \"$netemConfig\""
    done
  fi
  ;;
*)
