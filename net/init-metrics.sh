...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and AEKO_METRICS_CONFIG is
...
host="https://internal-metrics.aeko.com:8086"
...
if $useEnv; then
  [[ -n $AEKO_METRICS_CONFIG ]] ||
    usage "AEKO_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  AEKO_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export AEKO_METRICS_CONFIG=\"$AEKO_METRICS_CONFIG\"" >> "$configFile"
