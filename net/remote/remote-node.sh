    ...
    if [[ $skipSetup != true ]]; then
      ...
      setupValidatorKeypair() {
        ...
        if [[ -f net/keypairs/"$name".json ]]; then
          ...
          aeko-keygen new --no-passphrase -so config/"$name".json
          if [[ "$name" =~ ^validator-identity- ]]; then
            name="${name//-identity-/-vote-}"
            aeko-keygen new --no-passphrase -so config/"$name".json
            name="${name//-vote-/-stake-}"
        ...
        if [[ -n $internalNodesLamports ]]; then
          declare pubkey
          pubkey="$(aeko-keygen pubkey config/"$name".json)"
...
EOF
      ...
      for i in $(seq 0 $((numBenchTpsClients-1))); do
        ...
        aeko-bench-tps --write-client-keys config/bench-tps"$i".yml \
      ...
      if [[ "$extraPrimordialStakes" -gt 0 ]]; then
        ...
        for i in $(seq "$extraPrimordialStakes"); do
          args+=(--bootstrap-validator "$(aeko-keygen pubkey "config/validator-identity-$i.json")"
                                       "$(aeko-keygen pubkey "config/validator-vote-$i.json")"
                                       "$(aeko-keygen pubkey "config/validator-stake-$i.json")"
          )
        done
      fi
      ...
      if [[ -n "$maybeWarpSlot" ]]; then
        ...
        aeko-ledger-tool -l config/bootstrap-validator create-snapshot 0 config/bootstrap-validator $maybeWarpSlot
      fi

      aeko-ledger-tool -l config/bootstrap-validator shred-version --max-genesis-archive-unpacked-size 1073741824 | tee config/shred-version
    ...
    if [[ "$tmpfsAccounts" = "true" ]]; then
      args+=(--accounts /mnt/aeko-accounts)
    fi
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
        --blockstream /tmp/aeko-blockstream.sock
        --no-voting
        --dev-no-sigverify
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      aeko --url http://"$entrypointIp":8899 catchup config/validator-identity.json
