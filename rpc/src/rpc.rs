...
use {
    ...
    aeko_metrics::inc_new_counter_info,
    aeko_perf::packet::PACKET_DATA_SIZE,
    aeko_rpc_client_api::{
        config::*,
...
impl Default for RpcBigtableConfig {
    fn default() -> Self {
        let bigtable_instance_name = aeko_storage_bigtable::DEFAULT_INSTANCE_NAME.to_string();
        let bigtable_app_profile_id = aeko_storage_bigtable::DEFAULT_APP_PROFILE_ID.to_string();
        Self {
            enable_bigtable_ledger_upload: false,
            bigtable_instance_name,
            bigtable_app_profile_id,
            timeout: None,
            max_message_size: aeko_storage_bigtable::DEFAULT_MAX_MESSAGE_SIZE,
        }
    }
}
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<aeko_storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    pub fn new_from_bank(
        ...
        let cluster_info = Arc::new({
            ...
            let contact_info = ContactInfo::new_localhost(
                &keypair.pubkey(),
                aeko_sdk::timing::timestamp(), // wallclock
            );
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, aeko_storage_bigtable::Error>,
    ) -> Result<()> {
        if let Err(aeko_storage_bigtable::Error::BlockNotFound(slot)) = result {
    ...
    pub fn get_stake_activation(
        ...
        let stake_history =
            aeko_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
    ...
    fn get_stake_minimum_delegation(&self, config: RpcContextConfig) -> Result<RpcResponse<u64>> {
        ...
        let stake_minimum_delegation =
            aeko_stake_program::get_minimum_delegation(&bank.feature_set);
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        aeko_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == aeko_sdk::slot_history::Check::Found {
...
pub mod rpc_full {
    use {
        super::*,
        aeko_sdk::message::{SanitizedVersionedMessage, VersionedMessage},
        aeko_transaction_status::UiInnerInstructions,
    };
    #[rpc]
    pub trait Full {
        type Metadata;
...
pub fn create_test_transaction_entries(
    ...
    let success_tx = aeko_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair1.pubkey(),
        rent_exempt_amount,
        blockhash,
    ...
    let entry_1 = aeko_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx = aeko_sdk::system_transaction::transfer(
        keypair2,
        &keypair3.pubkey(),
    ...
    let entry_2 = aeko_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
...
pub fn populate_blockstore_for_tests(
    ...
    let shreds = aeko_ledger::blockstore::entries_to_test_shreds(
        &entries,
        slot,
        parent_slot,
        true,
    ...
    assert_eq!(
        aeko_ledger::blockstore_processor::process_entries_for_tests(
            &BankWithScheduler::new_without_scheduler(bank),
            entries,
            Some(
                &aeko_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    ...
    fn test_rpc_simulate_transaction_with_inner_instructions() {
        ...
        let lookup_table_space = aeko_sdk::address_lookup_table::state::LOOKUP_TABLE_META_SIZE;
        ...
        let (instruction, lookup_table_address) =
            aeko_sdk::address_lookup_table::instruction::create_lookup_table(
    ...
    #[test]
    fn test_token_rpcs() {
        for program_id in aeko_account_decoder::parse_token::spl_token_ids() {
            let rpc = RpcHandler::start();
            let bank = rpc.working_bank();
            ...
            let token_account_pubkey = aeko_sdk::pubkey::new_rand();
            let token_with_different_mint_pubkey = aeko_sdk::pubkey::new_rand();
            ...
            if program_id == inline_spl_token_2022::id() {
                ...
                let other_token_account_pubkey = aeko_sdk::pubkey::new_rand();
            ...
            let token_with_smaller_balance = aeko_sdk::pubkey::new_rand();
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base58 encoded aeko_sdk::transaction::Transaction too large: {tx58_len} bytes (max: encoded/raw {MAX_BASE58_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64, TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base64 encoded aeko_sdk::transaction::Transaction too large: {tx64_len} bytes (max: encoded/raw {MAX_BASE64_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "decoded aeko_sdk::transaction::Transaction too large: {too_big} bytes (max: {PACKET_DATA_SIZE} bytes)"
            ))
        );
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64.clone(), TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(
                "failed to deserialize aeko_sdk::transaction::Transaction: invalid value: \
