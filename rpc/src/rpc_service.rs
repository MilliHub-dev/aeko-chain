//! The `rpc_service` module implements the Aeko JSON RPC service.
...
use {
    ...
    aeko_perf::thread::renice_this_thread,
    ...
    aeko_send_transaction_service::send_transaction_service::{self, SendTransactionService},
    ...
    match path {
        "/v0/circulating-supply" => {
            ...
            let non_circulating_supply =
                aeko_runtime::non_circulating_supply::calculate_non_circulating_supply(&bank)
                    .expect("Scan should not error on root banks")
                    .lamports;
            Some(format!(
                "{}",
                lamports_to_aeko(total_supply - non_circulating_supply)
            ))
        }
        "/v0/total-supply" => {
            let bank = bank_forks.read().unwrap().root_bank();
            let total_supply = bank.capitalization();
            Some(format!("{}", lamports_to_aeko(total_supply)))
        }
        _ => None,
    }
}
...
impl JsonRpcService {
    ...
    pub fn new(
        ...
        let (bigtable_ledger_storage, _bigtable_ledger_upload_service) =
            ...
            {
                let bigtable_config = aeko_storage_bigtable::LedgerStorageConfig {
                ...
                runtime
                    .block_on(aeko_storage_bigtable::LedgerStorage::new_with_config(
                        bigtable_config,
                    ))
...
mod tests {
    use {
        ...
        aeko_rpc_client_api::config::RpcContextConfig,
    ...
    fn test_rpc_new() {
        ...
        let rpc_addr = SocketAddr::new(
            ip_addr,
            aeko_net_utils::find_available_port_in_range(ip_addr, (10000, 65535)).unwrap(),
        );
