use {
    ...
    aeko_sdk::{
        commitment_config::CommitmentConfig, message::Message, native_token::aeko_to_lamports,
...
fn create_messages(from: Pubkey, to: Pubkey) -> (Vec<Message>, f64) {
    ...
    for i in 1..NUM_TRANSACTIONS {
        ...
        let ix = system_instruction::transfer(&from, &to, aeko_to_lamports(amount_to_transfer));
...
fn test_send_and_confirm_transactions_in_parallel_without_tpu_client() {
    ...
    assert_eq!(
        rpc_client.get_version().unwrap().aeko_core,
        aeko_version::semver!()
    );
    ...
    assert_eq!(
        rpc_client
            ...
            .unwrap()
            .value,
        original_alice_balance - aeko_to_lamports(sum)
    );
}
