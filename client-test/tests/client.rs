use {
    ...
    aeko_ledger::{blockstore::Blockstore, get_tmp_ledger_path_auto_delete},
    aeko_pubsub_client::{nonblocking, pubsub_client::PubsubClient},
    aeko_rpc::{
...
fn test_rpc_client() {
    ...
    assert_eq!(
        client.get_version().unwrap().aeko_core,
        aeko_version::semver!()
    );
    ...
    let tx = system_transaction::transfer(&alice, &bob_pubkey, aeko_to_lamports(20.0), blockhash);
    ...
    assert_eq!(
        client
            ...
            .unwrap()
            .value,
        original_alice_balance - aeko_to_lamports(20.0)
    );
}
