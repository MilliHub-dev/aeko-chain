...
impl TryFrom<AccountSharedData> for VoteAccountInner {
    type Error = Error;
    fn try_from(account: AccountSharedData) -> Result<Self, Self::Error> {
        if !aeko_vote_program::check_id(account.owner()) {
...
mod tests {
    use {
        ...
        bincode::Options,
        rand::Rng,
        aeko_sdk::{pubkey::Pubkey, sysvar::clock::Clock},
        aeko_vote_program::vote_state::{VoteInit, VoteStateVersions},
