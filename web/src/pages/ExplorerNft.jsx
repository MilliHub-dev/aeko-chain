...
export default function ExplorerNft() {
  ...
  return (
    <div className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
      ...
      {!unavailable && state.loading ? <div className="text-gray-400">Loading NFT...</div> : null}
        ...
        <div className="space-y-8">
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-3">
              <Image className="h-6 w-6 text-aeko-accent" />
              <h1 className="text-2xl font-bold">AEKO-721 Token</h1>
            </div>
            <p className="font-mono text-sm text-gray-400 break-all">{nft.tokenId}</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Metric icon={Shield} label="Frozen" value={nft.frozen ? 'Yes' : 'No'} />
            <Metric icon={UserRound} label="Owner" value={nft.owner} />
            <Metric icon={UserRound} label="Creator" value={nft.creator} />
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            ...
            {nft.collectionId ? (
              <Link
                to={`/explorer/collection/${nft.collectionId}`}
                className="font-mono text-sm text-aeko-accent hover:text-white transition-colors break-all"
              >
                {nft.collectionId}
              </Link>
            ) : (
              <div className="font-mono text-sm text-gray-300 break-all">unlinked</div>
            )}
          </div>

          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
            <h2 className="text-lg font-bold mb-4">Metadata URI</h2>
            {nft.metadataUri ? (
              <a href={nft.metadataUri} target="_blank" rel="noreferrer" className="text-aeko-accent hover:text-white transition-colors break-all">
                {nft.metadataUri}
              </a>
            ) : (
              <div className="text-sm text-gray-500">No metadata URI indexed.</div>
            )}
          </div>
        </div>
      ) : null}
