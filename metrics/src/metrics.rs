...
use {
    ...
    log::*,
    aeko_sdk::{genesis_config::ClusterType, hash::hash},
    std::{
        cmp,
        collections::HashMap,
        convert::Into,
...
pub enum MetricsError {
    ...
    #[error("aeko_metrics_CONFIG is invalid: '{0}'")]
    ConfigInvalid(String),
    #[error("aeko_metrics_CONFIG is incomplete")]
    ConfigIncomplete,
    #[error("aeko_metrics_CONFIG database mismatch: {0}")]
    DbMismatch(String),
}
...
impl Default for MetricsAgent {
    fn default() -> Self {
        let max_points_per_sec = env::var("aeko_metrics_MAX_POINTS_PER_SECOND")
            .map(|x| {
                x.parse()
                    .expect("Failed to parse aeko_metrics_MAX_POINTS_PER_SECOND")
            })
            .unwrap_or(4000);
...
fn get_metrics_config() -> Result<MetricsConfig, MetricsError> {
    ...
    let config_var = env::var("aeko_metrics_CONFIG")?;
