...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $aeko_cli --keypair config/bootstrap-validator/identity.json \
...
killNodes() {
  ...
  echo "--- RPC exit"
  $aeko_validator --ledger "$AEKO_CONFIG_DIR"/bootstrap-validator exit --force || true
  $aeko_validator --ledger "$AEKO_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $aeko_ledger_tool --ledger "$AEKO_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $aeko_keygen new --no-passphrase -fso $client_keypair || exit $?
    $aeko_gossip --allow-private-addr spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
