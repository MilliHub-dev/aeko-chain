...
if [[ -n $CI ]]; then
  ...
  if [[ -n $TRAVIS ]]; then
    ...
    # to how aeko-ci-gate is used to trigger PR builds rather than using the
    ...
    *)
    ...
    if [[ -n $BUILDKITE_TRIGGERED_FROM_BUILD_PIPELINE_SLUG ]]; then
      # The aeko-secondary pipeline should use the slug of the pipeline that
      # triggered it
