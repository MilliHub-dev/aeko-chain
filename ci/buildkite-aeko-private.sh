...
#   cat  >> "$output_file" <<"EOF"
#   - name: "Trigger Build on aeko-secondary"
#     trigger: "aeko-secondary"
#     branches: "!pull/*"
#     async: true
#     build:
...
all_test_steps() {
  ...
  # Full test suite
  .buildkite/scripts/build-stable.sh aeko-private >> "$output_file"
...
EOF
  ...
  if affects \
      ...
      ; then
    .buildkite/scripts/build-downstream-projects.sh aeko-private >> "$output_file"
  else
    annotate --style info \
  ...
  if affects \
             ...
             ^ci/test-bench.sh \
      ; then
    .buildkite/scripts/build-bench.sh aeko-private >> "$output_file"
  else
...
#     "https://github.com/aeko-labs/aeko/releases/$BUILDKITE_TAG"
...
if [[ $BUILDKITE_BRANCH =~ ^pull ]]; then
  ...
  annotate --style info --context pr-backlink \
    "Github Pull Request: https://github.com/aeko-labs/aeko/$BUILDKITE_BRANCH"
