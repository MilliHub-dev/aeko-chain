...
cat >/etc/apt/apt.conf.d/99-aeko <<'EOF'
...
"$AEKO_ROOT"/net/scripts/install-docker.sh
usermod -aG docker "$SETUP_USER"
"$AEKO_ROOT"/net/scripts/install-certbot.sh
...
"$AEKO_ROOT"/net/scripts/install-earlyoom.sh
"$AEKO_ROOT"/net/scripts/localtime.sh
"$AEKO_ROOT"/net/scripts/install-rsync.sh
"$AEKO_ROOT"/net/scripts/install-libssl-compatability.sh
