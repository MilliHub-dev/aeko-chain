...
case $testName in
test-stable)
  _ ci/intercept.sh cargo test --jobs "$JOBS" --all --tests --exclude aeko-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-sbf)
  ...
  # aeko-keygen required when building C programs
  ...
  # SBF aeko-sdk legacy compile test
  ...
  # expected number of aeko-program crate compilations is 4. There
  # should be 3 builds of aeko-program while 128bit crate is
  ...
  tar cjvf "$sbf_dump_archive" "${sbf_target_path}"/{deploy/*.txt,sbf-aeko-aeko/release/*.so}
  exit 0
  ;;
test-stable-perf)
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for aeko-core unit tests
  ...
  _ cargo test --package aeko-perf --package aeko-ledger --package aeko-core --lib ${V:+--verbose} -- --nocapture
...
test-wasm)
  ...
  ;;
test-docs)
  _ cargo test --jobs "$JOBS" --all --doc --exclude aeko-local-cluster ${V:+--verbose} -- --nocapture
  exit 0
  ;;
*)
