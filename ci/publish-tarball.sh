...
RELEASE_BASENAME="${RELEASE_BASENAME:=aeko-release}"
...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/aeko-install-init aeko-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml aeko-install-init-"$TARGET"* $MAYBE_TARBALLS; do
  ...
  if [[ -n $BUILDKITE ]]; then
    ...
    upload-s3-artifact "/aeko/$file" s3://release.aeko.com/"$CHANNEL_OR_TAG"/"$file"

    echo Published to:
    $DRYRUN ci/format-url.sh https://release.aeko.com/"$CHANNEL_OR_TAG"/"$file"

    if [[ -n $TAG ]]; then
      ci/upload-github-release-asset.sh "$file"
    fi
  elif [[ -n $TRAVIS ]]; then
    # .travis.yml uploads everything in the travis-s3-upload/ directory to release.aeko.com
