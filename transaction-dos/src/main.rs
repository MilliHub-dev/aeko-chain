...
use {
    ...
    rand::{thread_rng, Rng},
    rayon::prelude::*,
    aeko_clap_utils::input_parsers::pubkey_of,
    aeko_cli::{
        cli::{process_command, CliCommand, CliConfig},
        program::ProgramCliCommand,
    },
    aeko_client::transaction_executor::TransactionExecutor,
    aeko_faucet::faucet::{request_airdrop_transaction, FAUCET_PORT},
    aeko_gossip::gossip_service::discover,
...
fn main() {
    ...
    if let Some(addr) = matches.value_of("entrypoint") {
        entrypoint_addr = aeko_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if let Some(addr) = matches.value_of("faucet_addr") {
        faucet_addr = aeko_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
            eprintln!("failed to parse entrypoint address: {e}");
            exit(1)
        });
    }
...
pub mod test {
    ...
    fn test_tx_size() {
        ...
        let signers: Vec<&Keypair> = vec![&keypair];
        let blockhash = aeko_sdk::hash::Hash::default();
        let tx = Transaction::new(&signers, message, blockhash);
